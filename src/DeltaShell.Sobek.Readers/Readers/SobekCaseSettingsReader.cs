...
namespace DeltaShell.Sobek.Readers.Readers
{
    ...
    public class SobekCaseSettingsReader
    {
        ...
        public static SobekCaseSettings GetSobekCaseSettings(string path)
        {
            ...
            var pumpSettings = new Dictionary<string, Action<string>>
                                        {
                                            {"PumpResults", v => settings.PumpResults = ConvertStringTo<bool>(v)}
                                        };
            ...
            var riverSettings = new Dictionary<string, Action<string>>
                                   {
                                       { "TransitionHeightSD", v => settings.TransitionHeightSummerDike = ConvertStringTo<double>(v) }
                                   };
            ...
            SetCategoryValues(source, "Simulation", simulationSettings);
            SetCategoryValues(source, "Timesteps", timeStepSettings);
            SetCategoryValues(source, "ResultsGeneral", generalSettings);
            SetCategoryValues(source, "ResultsNodes", nodeSettings);
            SetCategoryValues(source, "ResultsBranches", branchesSettings);
            SetCategoryValues(source, "ResultsStructures", structureSettings);

            // Test Check Presence of Category, If not Assignment of presentKeys fails
            if (source.Configs.OfType<IniConfig>().Any(c => c.Name == "ResultsPumps"))
            {
                SetCategoryValues(source, "ResultsPumps", pumpSettings);
            }

            SetCategoryValues(source, "InitialConditions", initialConditionsSettings);
            SetCategoryValues(source, "Flow Parameters", flowParameterSettings);
            SetCategoryValues(source, "River Options", riverSettings);
            SetCategoryValues(source, "Water Quality", waterQualitySettings);

            return settings;
        }

        private static void SetCategoryValues(IniConfigSource source, string categoryName, Dictionary<string, Action<string>> settings)
        {
            var category = source.Configs[categoryName];
            if (category == null)
            {
                Log.WarnFormat("Case settings for category [{0}] can not be found", categoryName);
                return;
            }

            var presentKeys = settings.Keys.Where(category.Contains).ToList();
            var missingKeys = settings.Keys.Except(presentKeys).ToList();

            foreach (var key in missingKeys)
            {
                Log.Warn(String.Format("Settings file {0} does not contain case setting {1}.", source.SavePath, key));
            }

            foreach (var key in presentKeys)
            {
                settings[key](category.Get(key));
            }
        }
