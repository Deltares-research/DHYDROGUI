...
namespace DeltaShell.Plugins.DelftModels.RTCShapes.Shapes
{
    public abstract class ShapeBase : Shape, IUnique<long>
    {
        ...
        {
            if (!RecalculateSize) return;

            var s = g.MeasureString(Title, Font);
        ...
        protected void PreRender(Graphics g)
        {
            ...
            switch (stringAlignment)
            {
                case StringAlignment.Center:
                    origin = new PointF(Rectangle.X + (Rectangle.Width / 2), Rectangle.Y + (Rectangle.Height / 2)); break;
                case StringAlignment.Far:
                    origin = new PointF(Rectangle.X + Rectangle.Width - 1, Rectangle.Y + 3); break;
                case StringAlignment.Near:
                    origin = new PointF(Rectangle.X + 1, Rectangle.Y + 3); break;
            }
        ...
        protected void RenderText(string text, PointF origin, Graphics g, float fontSize)
        {
            ...
            if (stringAlignment == StringAlignment.Center)
            {
                ...
                var stringSize = g.MeasureString(text, fontToMeasure);
