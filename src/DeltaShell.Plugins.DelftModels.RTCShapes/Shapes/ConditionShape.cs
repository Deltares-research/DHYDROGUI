...
namespace DeltaShell.Plugins.DelftModels.RTCShapes.Shapes
{
    ...
    public class ConditionShape : ShapeBase
    {
        ...
        {
            return RTCShapes.Properties.Resources.Condition;
        }
        ...
        private void RenderDescription(Graphics g)
        {
            ...
            if (Image != null)
            {
                var imageX = Rectangle.X + margin;
                var imageY = Rectangle.Bottom - (margin + Image.Height);
            ...
            {
                var description = GetDescriptionDelegate();

                var fontSize = Font.Size - 2;
                var fontToMeasure = new Font(Font.FontFamily, fontSize, Font.Style);
                var strRect = g.MeasureString(description, fontToMeasure);
        ...
        public override void Paint(Graphics g)
        {
            ...
            var pen = LeftNode.Connections.Count == 0 ? new Pen(Color.Black, 2) : Pens.Black;
