...
set mpiexedir=%D3D_HOME%\%ARCH%\mpiexec\bin
    ...
    rem Run
set PATH=%mpiexedir%;%dimrexedir%;%dflowfmexedir%;%flow1d2dexedir%;%rtcexedir%;%rrexedir%;%waveexedir%;%swanbatdir%;%swanexedir%;%esmfbatdir%;%esmfexedir%
    rem With debug info: "%dhydroexedir%\d_hydro.exe" -d 0xFFFFFFFF %argfile%
"%mpiexedir%\mpiexec.exe" -n %numpar% -localonly "%dimrexedir%\dimr.exe" %argfile%
