...
namespace DeltaShell.Plugins.NetworkEditor.MapLayers.Editors.Interactors
{
    ...
    public class DiffuseLateralSourceInteractor : LineStringInteractor, INetworkFeatureInteractor
    {
        ...
        public override void UpdateTracker(IGeometry geometry)
        {
            ...
            var index = 0;
            foreach (IGeometry trackerGeometry in Trackers.Select(t => t.Geometry))
            {
                                         ...
                                         SnapResult snapResult = null)
        {
            ...
            IBranch newBranch = snapResult == null ? lateralSource.Branch : (IBranch) snapResult.SnappedFeature;

            Coordinate oldLocation = trackerFeature == AllTracker
            ...
            int oldCoordinateCount = lateralSource.Geometry.Coordinates.Length;

            NetworkHelper.UpdateLineGeometry(lateralSource, newBranch.Geometry);

            int newCoordinateCount = lateralSource.Geometry.Coordinates.Length;
        ...
        {
            TrackerFeature trackerFeature = base.GetTrackerAtCoordinate(worldPos);
            if (trackerFeature == null)
            {
                return null;
            }

            foreach (TrackerFeature tracker in Trackers)
            {
