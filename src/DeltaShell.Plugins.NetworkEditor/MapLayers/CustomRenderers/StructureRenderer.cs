...
namespace DeltaShell.Plugins.NetworkEditor.MapLayers.CustomRenderers
{
    public class StructureRenderer : IFeatureRenderer, ICloneable, IDisposable
    {
        ...
        public bool Render(IFeature feature, Graphics g, ILayer layer)
        {
            ...
            IPoint point = GeometryFactory.CreatePoint(polygon.EnvelopeInternal.Centre);

            VectorStyle style = GetCurrentStyle(vectorLayer, feature);
            ...
            {
                int numberOfSameStructures = structure.ParentStructure.Structures.Count(s => s.GetType() == structure.GetType());
                if (numberOfSameStructures != 1)
                {
                    using (Graphics graphics = Graphics.FromImage(styleSymbol))
                    using (var font = new Font(FontFamily.GenericMonospace, 8, FontStyle.Regular))
                    {
                        ...
                        SizeF messageSize = graphics.MeasureString(numbOfStructuresString, font);
        ...
        private IGeometry GenerateCustomGeometry(IFeature feature, VectorLayer layer)
        {
            ...
            ICompositeBranchStructure compositeStructure = structure.ParentStructure;
            ...
            {
                IEnumerable<IGrouping<Type, IStructure1D>> grouping = compositeStructure.Structures.GroupBy(s => s.GetType());
                List<Type> structureGrouping = grouping.Select(t => t.Key).ToList();
        ...
        private static IGeometry GenerateCustomGeometry(IFeature feature, VectorLayer layer, int index, int structureFeatureCount)
        {
            ...
            VectorStyle style = GetCurrentStyle(layer, feature);

            Coordinate range = layer.Map.ImageToWorld(new PointF(style.Symbol.Width, style.Symbol.Height));
            Coordinate anchor = feature.Geometry.Coordinates[0];

            double halfWidth = (range.X - org.X) / 2;
            double d = ((anchor.X - (halfWidth * (structureFeatureCount - 1))) + (2 * halfWidth * index)) - halfWidth;
