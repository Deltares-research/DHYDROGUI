...
namespace DeltaShell.Plugins.NetworkEditor.Import
{
    public class HydroAreaEmbankmentHeightImporter : IFileImporter
    {
        ...
        public object ImportItem(string path, object target = null)
        {
            ...
            var totalNumberOfModelPoints = 0;
            ...
            if (numFailures == 0)
            {
                Log.Info("Successfully imported all height points");
            }
            else
            {
                Log.Warn("Successfully imported " + (shapefile.Features.Count - numFailures) + " of " + shapefile.Features.Count + " height points");
            }
        ...
        {
            for (var i = 1; i <= SizeOfLocalNeighbourhood; i++)
            {
        ...
        {
            for (var i = 0; i < targetHydroArea.Embankments.Count; i++)
            for (var j = 0; j < targetHydroArea.Embankments[i].Geometry.Coordinates.Length; j++)
            {
