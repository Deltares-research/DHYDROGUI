...
namespace DeltaShell.Plugins.FMSuite.FlowFM.IO.FouFile
{
    public static class FouFileReader
    {
        ...
        private static string IsFouFileUsed(string targetDir, WaterFlowFMModelDefinition modelDefinition)
        {
            ...
            if (!FouFileProperties.FouFileName.Equals(fouFileName, StringComparison.InvariantCultureIgnoreCase))
            {
                log.Warn($"FouFile name is {fouFileName}, expected default name: {FouFileProperties.FouFileName}. Using former to continue importing FouFile settings.");

                string filePath = Path.Combine(targetDir, fouFileName);
            ...
            string correctPath = Path.Combine(targetDir, FouFileProperties.FouFileName);
