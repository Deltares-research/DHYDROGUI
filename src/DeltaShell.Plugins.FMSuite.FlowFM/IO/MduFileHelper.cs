...
namespace DeltaShell.Plugins.FMSuite.FlowFM.IO
{
    public static class MduFileHelper
    {
        ...
        public static IList<string> GetMultipleSubfilePath(string mduFilePath, WaterFlowFMProperty property)
        {
            ...
            var simpleFilePath = GetSubfilePath(mduFilePath, property);
        ...
        private static void UpdateDefaultNamedFeatures<TFeat>(IList<TFeat> features, string extension, string defaultGroupName)
        {
            features.OfType<IGroupableFeature>()
                .Where(f => string.IsNullOrEmpty(f.GroupName) || f.GroupName.Equals(defaultGroupName) || f.IsDefaultGroup)
                .ForEach(f =>
                {
        ...
        /// <summary>
        /// Returns unique group names according to Windows. Windows does not make a distinction between uppercase and lowercase named
        /// files and directories. This method checks if a file with the same name already exists for all feature's group name. If so,
        ...
        /// <param name="alternativeExtensions">If a feature file has an alternative extension that it can save to, then this value is not null.</param>
        /// <returns>Unique relative file paths, according to Windows.</returns>
        public static string[] GetUniqueFilePathsForWindows<TFeat>(string targetMduFilePath, IList<TFeat> features, string extension, params string[] alternativeExtensions)
        {
            var mduDirectory = Path.GetFullPath(Path.GetDirectoryName(targetMduFilePath));
            var groupNames = features.OfType<IGroupableFeature>().Select(f => f.GroupName.Replace(@"\", "/")).Distinct().ToArray();
            ...
            groupNames = groupNames.Select(gn =>
                {
                    ...
                    if (fileExtension != null && gn.EndsWith(fileExtension) && !gn.EndsWith(extension))
                    {
            ...
            {
                var leadingValue = groupNames.Take(i).Where(
                    v => v.ToLowerInvariant() == groupNames[i].ToLowerInvariant() && v != groupNames[i]).Distinct().FirstOrDefault();
                if (leadingValue != null)
                {
                    Log.WarnFormat(Resources.MduFileHelper_GetUniqueFilePathsForWindows_Features_With_Group_Name___Are_Written_To_File___, groupNames[i], Path.Combine(mduDirectory, GetFilePathWithExtension(leadingValue, extension, alternativeExtensions)).Replace("/", @"\"));
        ...
        private static string GetFilePathWithExtension(string filePath, string extension, params string[] alternativeExtensions)
        {
            if (extension != null && filePath.EndsWith(Path.GetExtension(extension)) 
                || alternativeExtensions != null && alternativeExtensions.Any(filePath.EndsWith)) return filePath;
