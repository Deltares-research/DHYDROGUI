...
namespace DeltaShell.Plugins.FMSuite.FlowFM.IO
{
    public class FMMapFileFunctionStore : FMNetCdfFileFunctionStore
    {
        ...
        public IList<ITimeSeries> BoundaryCellValues { get; } = new List<ITimeSeries>();
        ...
        private void LogWarningsForExcludedTimeDependentVariables(IEnumerable<NetCdfVariableInfo> dataVariables)
        {
            ...
            List<NetCdfVariableInfo> filteredTimeDepVariables = dataVariables.Where(v => v.IsTimeDependent && v.NumDimensions > 2).ToList();
            IEnumerable<string> timeDepVariablesNames = filteredTimeDepVariables.Select(v => netCdfFile.GetVariableName(v.NetCdfDataVariable));
            ...
            IVariable function, IVariableFilter[] filters)
        {
            ...
            catch (Exception e) when (e.Message.Contains("NetCDF error code"))
            {
                ...
                return new MultiDimensionalArray<T>(new List<T>(new T[GetSize(function)]), GetSize(function));
            }
        }
