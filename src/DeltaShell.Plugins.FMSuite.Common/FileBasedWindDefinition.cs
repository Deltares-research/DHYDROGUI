    ...
    public class FileBasedWindDefinition
    {
        ...
        public static readonly IDictionary<WindDefinitionType, IList<FileBasedWindQuantity>> ExpectedQuantities =
            new Dictionary<WindDefinitionType, IList<FileBasedWindQuantity>>
                {
                    {
                        WindDefinitionType.WindXWindY,
                        new[] {FileBasedWindQuantity.VelocityX, FileBasedWindQuantity.VelocityY, FileBasedWindQuantity.AirPressure}
                    },
                    {WindDefinitionType.WindXY, new[] {FileBasedWindQuantity.VelocityVector, FileBasedWindQuantity.AirPressure}},
                    {WindDefinitionType.WindXYP, new[] {FileBasedWindQuantity.VelocityVectorAirPressure}},
                    {WindDefinitionType.SpiderWebGrid, new[] {FileBasedWindQuantity.SpiderWeb}}
                };
        ...
        public static readonly IDictionary<FileBasedWindQuantity, string[]> WindQuantityFileExtensions =
            new Dictionary<FileBasedWindQuantity, string[]>
                {
                    {FileBasedWindQuantity.VelocityX, new[] {UniformXSeriesFilter, XComponentArcInfoFilter}},
                    {FileBasedWindQuantity.VelocityY, new[] {UniformYSeriesFilter, YComponentArcInfoFilter}},
                    {FileBasedWindQuantity.VelocityVector, new[] {UniformXYSeriesFilter, UniformPolarSeriesFilter}},
                    {FileBasedWindQuantity.AirPressure, new[] {PressureComponentArcInfoFilter}},
                    {FileBasedWindQuantity.VelocityVectorAirPressure, new[] {MeteoCurviGridFilter}},
                    {FileBasedWindQuantity.SpiderWeb, new[] {SpiderWebFileFilter}}
                };
        ...
        public WindDefinitionType Type
        {
            ...
            set
            {
                var additionalSpiderWeb = AdditionalSpiderWeb;
                var spiderWebAdded = CanRemoveSpiderWeb;

                WindFiles.Clear();

                type = value;

                foreach (var quantity in ExpectedQuantities[type])
                {
        ...
        public bool CanAddSpiderWeb
        {
            get
            {
                return Type != WindDefinitionType.SpiderWebGrid && !WindFiles.ContainsKey(FileBasedWindQuantity.SpiderWeb);
            }
        }

        public bool CanRemoveSpiderWeb
        {
            get { return Type != WindDefinitionType.SpiderWebGrid && WindFiles.ContainsKey(FileBasedWindQuantity.SpiderWeb); }
        }
        ...
        {
            if (Type == WindDefinitionType.SpiderWebGrid) return;

            var spiderWebFile = WindFiles[FileBasedWindQuantity.SpiderWeb].FilePathHandler;
