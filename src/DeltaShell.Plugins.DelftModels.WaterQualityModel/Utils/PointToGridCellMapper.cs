...
namespace DeltaShell.Plugins.DelftModels.WaterQualityModel.Utils
{
    ...
    public class PointToGridCellMapper
    {
        ...
        /// <remarks>A height of 0 corresponds to the surface, where a height of 1 corresponds
        /// to the bottom.</remarks>
        /// <exception cref="ArgumentException">When the sum of all thicknesses in 
        /// <paramref name="relativeThicknesses"/> to not add up to ~1.0.</exception>
        ...
        /// </exception>
        /// <exception cref="System.ArgumentOutOfRangeException">When <paramref name="z"/>
        /// falls outside the valid range as defined when calling either <see cref="SetSigmaLayers"/>
        /// or <see cref="SetZLayers"/>.</exception>
        /// <exception cref="System.ArgumentException">When evaluating at a location outside
        /// the grid or when it is ambiguous in which cell it falls.</exception>
        public int GetWaqSegmentIndex(double x, double y, double z) // TODO: ICoordinate as parameter instead?
        {
            ...
            var relativeZ = MapToRelativeValue(z);
            if (!relativeZ.IsInRange(0.0, 1.0))
            {
                var message = string.Format("Height of point must be in range [{0}, {1}] for {2} models, but was {3}.", 
            ...
            var indexIn2DSpace = GetWaqSegmentIndex2D(x, y);
            if (indexIn2DSpace == 0)
            {
                throw new ArgumentException(string.Format("Point ({0}, {1}, {2}) is not within grid or has ambiguous location (on a grid edge or grid vertex).", x, y, z));
            }
            var index = GetLayerIndex(relativeZ);
        ...
        private int GetLayerIndex(double z)
        {
            ...
            var distance = 0.0;
            for (int i = 0; i < sigmaLayerThicknesses.Length; i++)
            {
            ...
            throw new InvalidOperationException("For z in range [0,1] and valid sigmaLayerThicknesses spanning to 1.0, a layer match should have been found.");
        }

        private void SetLayersCore(double[] relativeThicknesses, double topLevelValue, double bottomLevelValue, string layerType)
        {
            var sum = relativeThicknesses.Sum();
            if (Math.Abs(1.0 - sum) > 1.01e-3)
            {
                var message = string.Format("{0} layers should add up to ~1.0, but was adding up to {1}.", layerType, sum);
