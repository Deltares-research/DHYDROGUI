...
namespace DeltaShell.Plugins.FMSuite.Wave.IO
{
    public static class WaveBoundaryImportHelper
    {
        ...
        public static IGeometry CreateBoundaryGeometry(Coordinate startCoordinate, Coordinate endCoordinate, IList<double> condSpecAtDists)
        {
            IGeometry geometry;
            if (condSpecAtDists.Any())
            {
                var delta = Min(endCoordinate, startCoordinate);
                var length = Math.Sqrt(delta.X*delta.X + delta.Y*delta.Y);
                var coordinates = new List<Coordinate> {startCoordinate};

                const double eps = 1e-5;
                foreach (var condSpecAtDist in condSpecAtDists)
                {
            ...
            {
                geometry = new LineString(new[] {startCoordinate, endCoordinate});
            }
            return geometry;
        }
        ...
        { 
            var coordinates = Enumerable.Range(0, 2 + condSpecAtDists.Count())
                                        .Select(i => new Coordinate(i, 0))
                                        .OfType<Coordinate>()
                                        .ToArray();
            feature.Geometry = new LineString(coordinates);
            if (feature.Attributes == null) feature.Attributes = new DictionaryFeatureAttributeCollection();
            feature.Attributes["condSpecAtDists"] = condSpecAtDists.ToList();
        }

        public static IEnumerable<WaveBoundaryCondition> ConvertToCoordinateBased(IEnumerable<WaveBoundaryCondition> orientedBoundaryConditions, IDiscreteGridPointCoverage grid)
        {
            ...
            var boundaryDirections = CalculateSideOrientations(grid);

            foreach (var bc in orientedBoundaryConditions)
            {
                var dummyFeature = bc.Feature;
                var condSpecAtDists = (IList<double>)dummyFeature.Attributes["condSpecAtDists"];
                var direction = GetDirection((string)dummyFeature.Attributes["orientation"]);

                // match to side:
                var matchingSide = boundaryDirections.OrderBy(t => Math.Cos(direction - t.Item1)).Last();

                var start = matchingSide.Item2.First(c => !double.IsNaN(c.X) && !double.IsNaN(c.Y));
                var end = matchingSide.Item2.Last(c => !double.IsNaN(c.X) && !double.IsNaN(c.Y));
                ...
                Log.WarnFormat("Converting oriented boundary \'{0}\' to xy-coordinates, please check its location", bc.Name);
        ...
        public static List<Tuple<double, List<Coordinate>>> CalculateSideOrientations(IDiscreteGridPointCoverage grid)
        {
            ...
            sides.Add(Enumerable.Range(0, grid.Size1).Select(n => new Coordinate(x[nMax-n, mMax], y[nMax-n, mMax])).ToList());
            sides.Add(Enumerable.Range(0, grid.Size2).Select(m => new Coordinate(x[0, mMax-m], y[0, mMax-m])).ToList());
        ...
        private static double CalculateOrientation(List<Coordinate> coordinates)
        {
            ...
            for (int i = 1; i < coordinates.Count; ++i)
            {
                ...
                var x1 = coordinates[i-1].X;
                var y1 = coordinates[i-1].Y;
                var x2 = coordinates[i].X;
                var y2 = coordinates[i].Y;

                if (double.IsNaN(x1) || 
                    double.IsNaN(y1) ||
                    double.IsNaN(x2) || 
                    double.IsNaN(y2)) continue;
        ...
        {
            if (orientation == "north") return 0.0;
            if (orientation == "northwest") return Math.PI/4.0;
            if (orientation == "west") return Math.PI/2.0;
            if (orientation == "southwest") return 3.0*Math.PI/4.0;
            if (orientation == "south") return Math.PI;
            if (orientation == "southeast") return -3.0*Math.PI/4.0;
            if (orientation == "east") return -Math.PI/2.0;
            if (orientation == "northeast") return -Math.PI/4.0;
            return 0.0;
        }
    }
}