...
namespace DeltaShell.Plugins.FMSuite.Wave.IO.Importers
{
    public class WaveGridFileImporter : IFileImporter
    {
        ...
        public object ImportItem(string path, object target = null)
        {
            ...
            var model = getModels().First(m => WaveDomainHelper.GetAllDomains(m.OuterDomain).Any(d => Equals(d.Grid, targetGrid)));
            ...
            try
            {
                ...
                {
                    Log.ErrorFormat("Grid coordinates are incompatible with current model coordinate system: {0}, canceling import.", model.CoordinateSystem);
                    return null;
                }
