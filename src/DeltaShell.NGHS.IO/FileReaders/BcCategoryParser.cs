...
{
    /// <summary>
    /// <see cref="BcCategoryParser"/> is a helper class for parsing strings from Bc files.
    /// </summary>
    public class BcCategoryParser : IBcCategoryParser
    {
        private readonly ILogHandler logHandler;

        /// <summary>
        /// Creates a new <see cref="BcCategoryParser"/>.
        /// </summary>
        ...
        public BcCategoryParser(ILogHandler logHandler)
        {
        ...
        public bool TryParseDateTimes(IEnumerable<string> values, string unitValue, int lineNumber, out IEnumerable<DateTime> dateTimes)
        {
            ...
            logHandler.ReportError(string.Format(Resources.BcCategoryParser_TryParseDateTimes_Cannot_interpret___0____see_category_on_line__1__, unitValue, lineNumber));
            return false;
        }
        ...
        private bool TryGetReferenceTime(string unitValue, out DateTime referenceTime, int lineNumber)
        {
            ...
            if (!canParse)
            {
                logHandler.ReportError(string.Format(Resources.BcCategoryParser_TryGetReferenceTime_Cannot_parse___0___to_a_date_time__see_category_on_line__1__, dateTimeString, lineNumber));
        ...
        private bool TryParseDouble(string doubleString, int lineNumber, out double doubleVal)
        {
            ...
            if (!canParse)
            {
                logHandler.ReportError(string.Format(Resources.BcCategoryParser_TryParseDouble_Cannot_parse___0___to_a_double__see_category_on_line__1__, doubleString, lineNumber));
            }
            return canParse;
        }

        public double CreateConstant(IList<IDelftBcQuantityData> table, int lineNumber)
        {
            Ensure.NotNull(table, nameof(table));

            if (!TableHasValue(table))
            {
                logHandler.ReportError(string.Format(Resources.BcCategoryParser_Table_on_line__0___does_not_contain_any_values, lineNumber));
        ...
        private static bool TableHasValue(IList<IDelftBcQuantityData> table)
        {
