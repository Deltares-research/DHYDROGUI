...
namespace DelftTools.Hydro.Helpers
{
    public static class ThreadingHelper
    {
        ...
        public static IList<TR> ConvertMultiThreaded<TL, TR>(this IList<TL> originalItems, Func<TL, TR> action,
                                                             int numberOfThreads = -1)
        {
            numberOfThreads = numberOfThreads == -1
                                  ? Math.Min(10, Math.Max(1, originalItems.Count / 500))
                                  : numberOfThreads;
            ...
            {
                int numberOfElementsToTake = i == numberOfThreads - 1
                                                 ? originalItems.Count - (i * numberOfElementsPerThread)
                                                 : numberOfElementsPerThread;

                List<TL> itemsForThread =
                    originalItems.Skip(i * numberOfElementsPerThread).Take(numberOfElementsToTake).ToList();
                int i1 = i;
