...
namespace DeltaShell.Plugins.DelftModels.WaterFlowModel.ImportExport.Structures
{
    public class OrificeConverter : StructureConverter
    {
        ...
        protected override void SetStructurePropertiesFromCategory(IList<string> warningMessages)
        {
            ...
            if (!(weir.WeirFormula is GatedWeirFormula weirFormula)) return;
            ...
            weirFormula.ContractionCoefficient = Category.ReadProperty<double>(StructureRegion.ContractionCoeff.Key);
            weirFormula.LateralContraction = Category.ReadProperty<double>(StructureRegion.LatContrCoeff.Key);
            weirFormula.GateOpening = Category.ReadProperty<double>(StructureRegion.OpenLevel.Key) - weir.CrestLevel;
            weirFormula.UseMaxFlowPos = Convert.ToBoolean(Category.ReadProperty<int>(StructureRegion.UseLimitFlowPos.Key));
            weirFormula.UseMaxFlowNeg = Convert.ToBoolean(Category.ReadProperty<int>(StructureRegion.UseLimitFlowNeg.Key));
