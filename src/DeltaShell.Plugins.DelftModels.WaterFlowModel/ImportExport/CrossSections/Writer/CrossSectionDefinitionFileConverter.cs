    ...
    public class CrossSectionDefinitionFileConverter
    {
        public virtual IEnumerable<DelftIniCategory> Convert(WaterFlowModel1D waterFlowModel1D)
        {
            ...
            foreach (var crossSection in crossSections)
            {
                ...
                foreach (var singleCrossSection in crossSections)
                {
                    var name = singleCrossSection.Definition.Name;
                    definitions.Add(name);
                }

                var hasSharedDefinition = false;
                foreach (var sharedCrossSection in sharedCrossSections)
                {
                    hasSharedDefinition = definitions.Any(d => d.Contains(sharedCrossSection.Name));
                }

                if (sharedCrossSections.Contains(definition) || hasSharedDefinition)
                {
