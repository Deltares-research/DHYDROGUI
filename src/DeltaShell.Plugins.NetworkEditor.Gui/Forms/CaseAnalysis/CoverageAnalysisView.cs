...
namespace DeltaShell.Plugins.NetworkEditor.Gui.Forms.CaseAnalysis
{
    ...
    public partial class CoverageAnalysisView : UserControl, ICompositeView
    {
        ...
        {
            IEnumerable<IModel> models = data.GetAllItemsRecursive().OfType<IModel>().Where(model => !(model is ICompositeActivity));

            IModel parentModel = models.FirstOrDefault(m => m.GetAllItemsRecursive().OfType<INetworkCoverage>().Contains(item));
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            {
                foreach (CoverageWrapper coverageWrapper in primaryCoverages)
                {
                    coverageWrapper.Dispose();
                }

                foreach (CoverageWrapper coverageWrapper in secondaryCoverages)
                {
                    coverageWrapper.Dispose();
                }
        ...
        private void FillOperationsList()
        {
            ...
            foreach (NetworkCoverageOperations.INetworkCoverageOperation operation in GetOperations())
            {
        ...
        private void ProjectCollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            ...
            {
                foreach (INetworkCoverage cov in coverages)
                {
        ...
        private void OnNetworkCoverageAdded(INetworkCoverage cov)
        {
            ...
            CoverageWrapper wrapper = CreateWrapper(cov);
        ...
        {
            CoverageWrapper wrapper = primaryCoverages.FirstOrDefault(cw => ReferenceEquals(cw.Coverage, cov));
        ...
        {
            INetworkCoverage processedCoverage = GetProcessedCoverageFromSelections();
        ...
        private INetworkCoverage GetProcessedCoverageFromSelections()
        {
            ...
            INetworkCoverage secondaryCoverage = operation.RequiresSecondCoverage
        ...
        private bool ShowWarningSecondaryCoverage(bool secondaryCoverageRequired)
        {
            ...
            INetworkCoverage secondaryCoverage = secondaryCoverageRequired
                                                     ? comboboxSecondary.SelectedValue as INetworkCoverage
                                                     : null;
        ...
        public object Data
        {
            ...
            set
            {
                ...
                {
                    ((INotifyCollectionChanged) data).CollectionChanged += ProjectCollectionChanged;
                    foreach (INetworkCoverage cov in data.GetAllItemsRecursive().OfType<INetworkCoverage>())
                    {
                        OnNetworkCoverageAdded(cov);
                    }
                }
