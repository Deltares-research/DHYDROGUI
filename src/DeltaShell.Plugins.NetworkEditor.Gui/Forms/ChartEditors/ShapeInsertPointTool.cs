    ...
    public class ShapeInsertPointTool : ShapeLayerTool
    {
        ...
        public override void Paint()
        {
            if (SnapResult != null)
            {
                ...
                var vectorStyle2Graphics3D = new ChartDrawingContext(ShapeModifyTool.Chart.Graphics, vectorStyle);
                for (var i = 0; i < SnapResult.VisibleSnaps.Count; i++)
                {
        ...
        public override void MouseEvent(ChartMouseEvent kind, MouseEventArgs e, Cursor c)
        {
            if (null != ShapeModifyTool.ShapeFeatureEditor)
            {
                ...
                var coordinate = new Coordinate(ChartCoordinateService.ToWorldX(ShapeModifyTool.Chart, tmP.X), ChartCoordinateService.ToWorldX(ShapeModifyTool.Chart, tmP.Y));
