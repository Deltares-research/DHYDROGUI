...
namespace DeltaShell.Plugins.NetworkEditor.Gui.VectorAttributeTableViewCreation.CreationContexts.Features1D
{
    ...
    public class SewerConnectionTableViewCreationContext : ITableViewCreationContext<ISewerConnection, SewerConnectionRow, IHydroNetwork>
    {
        ...
        /// <inheritdoc/>
        public SewerConnectionRow CreateFeatureRowObject(ISewerConnection feature)
        {
            Ensure.NotNull(feature, nameof(feature));
            return new SewerConnectionRow(feature);
        }
        ...
        {
            IModelWithNetwork networkModel = guiContainer.Gui.Application.GetAllModelsInProject().OfType<IModelWithNetwork>().FirstOrDefault(m => m.Network.SewerConnections.Equals(data));
