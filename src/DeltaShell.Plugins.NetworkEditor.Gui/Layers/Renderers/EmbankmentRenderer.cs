...
namespace DeltaShell.Plugins.NetworkEditor.Gui.Layers.Renderers
{
    public class EmbankmentRenderer : IFeatureRenderer
    {
        ...
        public bool Render(IFeature feature, Graphics g, ILayer layer)
        {
            ...
            double[] allValues = geometry.Coordinates.Select(c => c.Z).ToArray();
            ...
            {
                Coordinate coordinateBegin = coordinates[i];
                Coordinate coordinateEnd = coordinates[i + 1];
                ...
                Envelope viewEnvelope = layer.Map.Envelope;
        ...
        {
            var a = Convert.ToByte(color1.A + (proportion * (color2.A - color1.A)));
            var r = Convert.ToByte(color1.R + (proportion * (color2.R - color1.R)));
            var g = Convert.ToByte(color1.G + (proportion * (color2.G - color1.G)));
            var b = Convert.ToByte(color1.B + (proportion * (color2.B - color1.B)));
