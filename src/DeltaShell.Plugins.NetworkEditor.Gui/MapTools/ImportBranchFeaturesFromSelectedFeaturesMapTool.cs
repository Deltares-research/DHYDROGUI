...
namespace DeltaShell.Plugins.NetworkEditor.Gui.MapTools
{
    public class ImportBranchFeaturesFromSelectedFeaturesMapTool : MapTool
    {
        ...
        public override bool Enabled
        {
            get
            {
                return HydroNetworkMapLayer != null && MapControl.SelectedFeatures.Any(s => s.Geometry is Point && !(s is IHydroObject));
            }
        }
        ...
        public void ImportSelectedFeaturesAsBranchFeatures(ILayer branchFeatureLayer)
        {
            ...
            foreach (IFeature feature in MapControl.SelectedFeatures)
            {
                ...
                IBranch nearestBranch = NetworkHelper.GetNearestBranch(network.Branches, feature.Geometry, Tolerance);

                if (nearestBranch != null)
                {
                    IFeatureProvider featureProvider = branchFeatureLayer.DataSource;

                    // calculate new branch feature location
                    Coordinate coordinate = GeometryHelper.GetNearestPointAtLine((ILineString) nearestBranch.Geometry,
                    ...
                    string nameAttribute = feature.Attributes.Keys.FirstOrDefault(a => a.ToUpper().StartsWith("NAME"));
        ...
        public override void Execute()
        {
            ...
            foreach (ILayer layer in HydroNetworkMapLayer.Layers)
            {
