...
namespace DeltaShell.Plugins.DelftModels.WaterQualityModel.Gui.Forms
{
    ...
    public partial class FindGridCellDialog : Form
    {
        ...
        {
            var parsedIndex = -1;
            var hasValidIndex = int.TryParse(gridIndexTextBox.Text, out parsedIndex) && parsedIndex >= 1 && parsedIndex <= maximumGridCellId;

            var errorMessage = string.Format("Value must be a whole number in the range [{0}, {1}]", 1, maximumGridCellId);
