    ...
    public partial class BloomFunctionsTableView : UserControl, IView
    {
        ...
        public BloomFunctionsTableView()
        {
            ...
            functionCollectionChangedDelayedEventHandler =
                new DelayedEventHandler<EventArgs>(delegate { UpdateTableColumns(); UpdateTableView(); })
                {
        ...
        private void UpdateTableColumns()
        {
            ...
            {
                var prettyHeader = string.Format("{0}[{1}]", header.Substring(0, header.Length - 3),header.Substring(header.Length - 3));
                var resultColumn = tableView.AddUnboundColumn(prettyHeader, typeof (double));
        ...
        private object UnboundColumnData(int columnIndex, int rowIndex, bool isGetter, bool isSetter, object value)
        {
            ...
            if (isGetter)
            {
                ...
                if (columnIndices.TryGetValue(columnIndex, out columnHeader))
                {
                    ...
                    var function = functions.FirstOrDefault(f => string.Equals(f.Name, funcName, StringComparison.InvariantCultureIgnoreCase));
            ...
            if (isSetter)
            {
                ...
                if (!columnIndices.TryGetValue(columnIndex, out columnHeader)) return null;

                var funcName = info.MakeParameter(columnHeader, kortName);
                var function = functions.FirstOrDefault(f => string.Equals(f.Name, funcName, StringComparison.InvariantCultureIgnoreCase));
