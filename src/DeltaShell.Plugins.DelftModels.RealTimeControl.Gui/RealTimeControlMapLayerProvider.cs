...
namespace DeltaShell.Plugins.DelftModels.RealTimeControl.Gui
{
    public class RealTimeControlMapLayerProvider : IMapLayerProvider
    {
                                             ...
                                             Style = new VectorStyle
                                                         {
                                                             ...
                                                             Line = new Pen(new SolidBrush(ColorHelper.GetIndexedColor(100, 0)), 8)
                                                                        {
        ...
        public IEnumerable<object> ChildLayerObjects(object data)
        {
            ...
            if (realTimeControlModel != null)
            {
                yield return new ModelFolder {Model = realTimeControlModel, Role = DataItemRole.Input};
                yield return new ModelFolder {Model = realTimeControlModel, Role = DataItemRole.Output};
            }
            ...
            if (modelfolder != null && modelfolder.Model is RealTimeControlModel)
            {
                ...
                {
                    foreach (var outputFeatureCoverage in rtcModel.OutputFeatureCoverages)
                    {
