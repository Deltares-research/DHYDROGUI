...
namespace DeltaShell.NGHS.Common.Tests.IO.Restart
{
    ...
    public class RestartFileTest
    {
        ...
        [TestCase(false, null)]
        [TestCase(false, "")]
        public void CopyTo_DirectoryPathNullOrEmpty_Returns(bool switchTo, string targetFile)
        {
            // Setup
            using (var tempDir = new TemporaryDirectory())
            {
                ...
                // Call
                restartFile.CopyTo(targetFile, switchTo);
        ...
        [TestCase(true)]
        [TestCase(false)]
        public void CopyTo_FileDoesNotExist_Returns(bool switchTo)
        {
            // Setup
            var restartFile = new RestartFile("path/to/the.file");

            // Call
            restartFile.CopyTo("some/folder/the.file", switchTo);
        ...
        public void CopyTo_TargetDirDoesNotExist_CreatesTargetDir(bool switchTo)
        {
            ...
            using (var tempDir = new TemporaryDirectory())
            {
                ...
                string targetFile = Path.Combine(tempDir.Path, "target_dir", restartFile.Name);
        ...
        [TestCase(true)]
        [TestCase(false)]
        public void CopyTo_CopiesFile(bool switchTo)
        {
            // Setup
            using (var tempDir = new TemporaryDirectory())
            {
                ...
                string targetFile = Path.Combine(tempDir.CreateDirectory("target_dir"), restartFile.Name);
