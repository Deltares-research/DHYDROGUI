...
namespace DelftTools.Hydro.Tests.Helpers
{
    ...
    public class CrossSectionHelperTest
    {
        ...
        public void SetDefaultGeometryWithBrancheGeometry()
        {
            ...
            var offset = 12;
            ICrossSection crossSection = HydroNetworkHelper.AddCrossSectionDefinitionToBranch(channel, crossSectionDefinition, offset);
            ...
            Assert.AreEqual(new[]
            {
                2d,
                -2d
            }, crossSection.Geometry.Coordinates.Select(c => c.Y).ToList());
        }
        ...
        public void YZTableToPerpendicularGeometry()
        {
            ...
            double minY = yzCoordinates.Min(c => c.X);
            double maxY = yzCoordinates.Max(c => c.X);
            IGeometry geometry = CrossSectionHelper.CreatePerpendicularGeometry(branchGeometry, 30, minY, maxY, thalWegOffset);
            var expectedGeometry = new LineString(new[]
            {
                new Coordinate(30, -10, 0),
                new Coordinate(30, -5, -20),
                new Coordinate(30, 5, -20),
                new Coordinate(30, 10, 0)
            });
            Assert.IsTrue(expectedGeometry.Equals(geometry));
        }

        [Test]
        public void AreaOfEmptyProfiles()
        {
            var upper = new Coordinate[]
                {};
            var lower = new Coordinate[]
                {};
        ...
        public void CreatePerpendicularGeometry()
        {
            ...
            IGeometry perpendicularGeometry = CrossSectionHelper.CreatePerpendicularGeometry(branchGeometry, offsetAlongBranch, minY, maxY, 0.0);

            //compare the calculated geometry with a small tolerance. Rounding errors occur due to Sin/Cos
            var expected = new LineString(new[]
            {
