...
{
    public static class WaterFlowModel1DDemoModelTestHelper
    {
        public static void ConfigureModelAsDemoModel(WaterFlowModel1D model, bool addCrossSections = true)
        {
            ConfigureDemoNetwork(model.Network, addCrossSections);
        ...
        private static void ConfigureAsDemoModel(WaterFlowModel1D model)
        {
            var network = model.Network;

            model.Name = "flow model 1d (demo network)";
        ...
        public static IHydroNetwork CreateLargeNetwork(CrossSectionSectionType crossSectionSectionType, int cellCount)
        {
        ...
        public static void ReplaceStoreForOutputCoverages(WaterFlowModel1D flowModel1D, bool generateUniqueBranchIds = true, bool convertFeatureCoveragesToo = true)
        {
            if (generateUniqueBranchIds)
            {
            ...
            foreach (var pi in flowModel1D.GetAllItemsRecursive().OfType<DataItem>()
                                .Where(di => (convertFeatureCoveragesToo ? di.Value is INetworkCoverage || di.Value is IFeatureCoverage : di.Value is INetworkCoverage) &&
