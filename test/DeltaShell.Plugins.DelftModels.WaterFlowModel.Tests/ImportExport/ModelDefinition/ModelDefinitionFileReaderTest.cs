...
namespace DeltaShell.Plugins.DelftModels.WaterFlowModel.Tests.ImportExport.ModelDefinition
{
    ...
    public class ModelDefinitionFileReaderTest
    {
        ...
        public void GivenDataModelWithCategoryThatHasAnUnknownHeader_WhenSettingProperties_ThenLogMessageIsReturned()
        {
            ...
            {
                // When
                ModelDefinitionFileReader.SetWaterFlowModelProperties(testFilePath, new WaterFlowModel1D(),
                    CreateAndAddErrorReport);

                // Then
                var expectedMessage = string.Format(
                    Resources
                        .ModelDefinitionFileReaderTest_GivenDataModelWithCategoryThatHasAnUnknownHeader_WhenSettingProperties_ThenLogMessageIsReturned_There_is_unrecognized_data_read_from_the_md1d_file_with_header__0___This_category_has_been_skipped_when_reading_,
                    "Unknown Header");
        ...
        [Category(TestCategory.DataAccess)]
        public void
            GivenAWaterFlow1DOutputSettingDataWithAllEngineParametersSetToNoneAndADataAccessModelDescribingMultipleEngineParameterWithSomeAggregateOptionsNotNone_WhenWaterFlowModelPropertySetterSetOutputPropertiesIsCalledWithTheseParameters_ThenTheseEnginePropertyIsSetToTheSpecifiedAggregateOptionAndAllOtherEngineParametersAreNone()
        {
            // Given 
            ...
            foreach (var t in resultsNodesProperties)
                Assert.That(
                    outputSettings.GetEngineParameter(t.Key, ElementSet.GridpointsOnBranches).AggregationOptions,
                    Is.EqualTo(t.Value), $"Property name: {t.Key.ToString()}");

            // Check corner case: Lateral1D2D
            Assert.That(
                outputSettings.GetEngineParameter(QuantityType.QTotal_1d2d, ElementSet.GridpointsOnBranches)
                    .AggregationOptions,
            ...
            Assert.That(
                outputSettings.GetEngineParameter(QuantityType.Dispersion, ElementSet.GridpointsOnBranches)
                    .AggregationOptions,
                Is.EqualTo(AggregationOptions.None));
            ...
            Assert.That(errorHandlingHasBeenCalled, Is.False,
                "Expected no calls to the error handling, but found at least one.");
        }

        [Test]
        public void GivenAWaterFlowModel1DWithInvalidFilePath_WhenReadingCategoriesFromFileAndCollectErrorMessages_ThenExceptionIsGiven()
        {
            // Given 
            var loggedErrors = new List<string>();
            ...
            // When/Then
            var testFilePath = "invalidFilePath";
            ModelDefinitionFileReader.SetWaterFlowModelProperties(testFilePath, model, someErrorReportFunction);

            Assert.That(loggedErrors.Count, Is.EqualTo(1));
            Assert.That(loggedErrors.ElementAt(0), Is.EqualTo("Could not read file invalidFilePath properly, it doesn't exist."));
        }
    }
}