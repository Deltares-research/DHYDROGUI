...
namespace DeltaShell.Plugins.DelftModels.WaterFlowModel.Tests.ImportExport.CrossSections.Writer
{
    ...
    public class CrossSectionDefinitionFileConverterTest
    {
        ...
        public void GivenWaterFlow1DModelWithCrossSectionDependedUponSharedCrossSectionDefinition_WhenConverting_ThenIsShared2()
        {
            ...
            mockedCrossSection.Expect(mcsd => mcsd.UseSharedDefinition(new CrossSectionDefinitionYZ()));
            mockedCrossSection.Expect(mcs => mcs.Definition).Return(crossSectionDefinition);
            mockedModel.Network.SharedCrossSectionDefinitions.Add(crossSectionDefinition);
            ...
            mockedCrossSection2.Expect(mcsd => mcsd.UseSharedDefinition(new CrossSectionDefinitionYZ()));
            mockedCrossSection2.Expect(mcs => mcs.Definition).Return(crossSectionDefinition);
            ...
            mockedCulvert.Expect(m => m.CrossSectionDefinition).Return(new CrossSectionDefinitionProxy(new CrossSectionDefinitionYZ()));
            ...
            // When
            var categories = new CrossSectionDefinitionFileConverter().Convert(mockedModel).ToList();
            
            //Then
            Assert.That(categories.Count(), Is.EqualTo(2));
            Assert.That(categories.ElementAt(1).Properties.Last().Name, Is.EqualTo("isShared"));
            Assert.That(categories.ElementAt(1).Properties.Last().Value, Is.EqualTo("1"));
        }
    }
}
