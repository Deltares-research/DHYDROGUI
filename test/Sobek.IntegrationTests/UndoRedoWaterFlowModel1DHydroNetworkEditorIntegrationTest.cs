...
namespace Sobek.IntegrationTests
{
    ...
    public class UndoRedoWaterFlowModel1DHydroNetworkEditorIntegrationTest : UndoRedoHydroRegionTestBase
    {
        ...
        public void SaveLoadModelAndAddCulvert()
        {
            onMainWindowShown = () =>
            {
                ...
                var path = "SaveLoadModelAndAddCulvert.dsproj";
                ...
                model = (WaterFlowModel1D)gui.Application.Project.RootFolder.Models.First();
                network = model.Network;
            ...
            onMainWindowShown = () =>
                {
                    ...
                    ProjectItemMapView = gui.DocumentViews.OfType<ProjectItemMapView>().Last();
        ...
        public void UndoSettingCustomLengthOnBranch_JIRA9127()
        {
            onMainWindowShown = () =>
            {
                var channel = AddBranchToNetwork(new[] { new Coordinate(0, 0), new Coordinate(100, 0) });
                channel.IsLengthCustom = true;
                channel.Length = 200.0;
                
                network.BeginEdit(new DefaultEditAction("set length"));
                channel.IsLengthCustom = false;

                // before setting Length to 500.0, OnPropertyChanging stores the
                // current property value by calling get_Length, which now gives 
                // the geometry length 100.0 instead of 200.0 because IsLengthCustom 
                // was set to false...

                channel.Length = 500.0;
                channel.IsLengthCustom = true;
                network.EndEdit();

                gui.UndoRedoManager.Undo();

                Assert.IsTrue(channel.IsLengthCustom);
                Assert.AreEqual(200.0, channel.Length);
            };
