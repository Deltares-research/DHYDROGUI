...
namespace Sobek.IntegrationTests
{
    ...
    public class UndoRedoHydroRegionEditorNetworkIntegrationTest : UndoRedoHydroRegionTestBase
    {
        [SetUp]
        public void SetUp()
        {
        ...
        public void SplitBranchWithRouteAfterSave()
        {
            onMainWindowShown = () =>
            {
                ...
                // save project
                gui.Application.SaveProjectAs("temp.dsproj");
            ...
            onMainWindowShown = () =>
                {
                    ...
                    var csDef = new CrossSectionDefinitionXYZ("cs1")
                        {
                            Geometry = new LineString(new []
                                {
                                    new Coordinate(250, -20, 0), new Coordinate(250, -10, -3), new Coordinate(250, 0, -5), new Coordinate(250, 10, -3), new Coordinate(250, 20, 0)
                                })
                        };
                    csDef.XYZDataTable[0].DeltaZStorage = 15.0;
                    var cs = new CrossSection(csDef);

                    NetworkHelper.AddBranchFeatureToBranch(cs, branch, 250);

                    MoveFeature(cs, new Coordinate(300, 10));

                    Assert.AreEqual(300, cs.Geometry.Coordinates[0].X);
                    Assert.AreEqual(15.0, csDef.XYZDataTable[0].DeltaZStorage);
                    
                    gui.UndoRedoManager.Undo();

                    Assert.AreEqual(250, cs.Geometry.Coordinates[0].X);
                    Assert.AreEqual(15.0, csDef.XYZDataTable[0].DeltaZStorage);
                };
                    ...
                    Geometry = new LineString(new []
                                {
                                    ...
                                    new Coordinate(250, -10, -3), 
                                    new Coordinate(250, 0, -5), 
                                    new Coordinate(250, 10, -3), 
                                    new Coordinate(250, 20, 0)
                                })
                };
                csDef.XYZDataTable[3].DeltaZStorage = 15.0;
                var cs = new CrossSection(csDef);

                NetworkHelper.AddBranchFeatureToBranch(cs, branch, 250);

                RemoveCoordinateFromFeatureGeometry(cs, new Coordinate(250, 0, -5));

                Assert.AreEqual(4, cs.Geometry.Coordinates.Count());
                Assert.AreEqual(0.0, csDef.XYZDataTable[3].DeltaZStorage);

                gui.UndoRedoManager.Undo();

                Assert.AreEqual(5, cs.Geometry.Coordinates.Count());
        ...
        public void UndoAddRoute()
        {
            onMainWindowShown = () =>
            {
                ...
                Assert.AreEqual(0, ProjectItemMapView.MapView.Map.GetAllLayers(true).OfType<NetworkCoverageGroupLayer>().Count(
                ...
                Assert.AreEqual(1, ProjectItemMapView.MapView.Map.GetAllLayers(true).OfType<NetworkCoverageGroupLayer>().Count(
                                    l => ReferenceEquals(l.Coverage, route)));
            };
