...
namespace DeltaShell.Plugins.DelftModels.RealTimeControl.Tests.Xml
{
    ...
    public class XmlTimeSeriesTests
    {
        ...
        {
            var timeSeries = xmlTimeSeries.TimeSeries;
            timeSeries.Components.Clear();

            var lstEvenIndexIsTrue = new List<bool>();
            for (int i = 0; i < timeSeries.Time.Values.Count; i++)
            {
            ...
            var boolComponent = new Variable<bool> {Name = "Value", DefaultValue = false, NoDataValue = false, IsAutoSorted = false};
