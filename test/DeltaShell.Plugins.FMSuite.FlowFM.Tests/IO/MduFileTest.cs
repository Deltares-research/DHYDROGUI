    ...
    public class MduFileTest
    {
        ...
        public void WriteExternalForcingsFiles()
        {
            ...
            TestHelper.PerformActionInTemporaryDirectory(tempDir =>
            {
                ...
                mduFile.Write(testFile, modelDefinition, hydroArea, null, null,null, null, null, null,new List<ModelFeatureCoordinateData<FixedWeir>>(), false, true, false);
        ...
        public void WriteMorphologyAndSedimentFiles()
        {
            ...
            mduFile.Write(testFile, modelDefinition, hydroArea, null, null, null, null, null, null, new List<ModelFeatureCoordinateData<FixedWeir>>(), false,false, true, false, sedimentData);
        ...
        public void Test_MduFile_Read_Loads_BridgePillars()
        {
            ...
            convertedFileObjectsForFMModel.AllBridgePillarsAndCorrespondingProperties = null;
            
            mduFile.Read(testPath, convertedFileObjectsForFMModel);
            Assert.IsTrue(convertedFileObjectsForFMModel.HydroArea.BridgePillars.Any());

            var pillar = convertedFileObjectsForFMModel.HydroArea.BridgePillars.First();
            ...
            Assert.AreEqual(2, pillar.Attributes.Count);
        ...
        public void Test_MduFile_Write_Writes_BridgePillars_Entry()
        {
            ...
            try
            {
                mduFile.Write(testFile, modelDefinition, hydroArea, null,null, null, null, null, null, new List<ModelFeatureCoordinateData<FixedWeir>>());
            }
            catch (Exception e )
            {
        ...
        public void MduFile_SetBridgePillarDataModel_WithTooManyColumns_DoesNotCrash()
        {
            ...
            var mduFile = new MduFile();
            var convertedFileObjectsForFMModel = CreateConvertedFileObjectsForFMModel();
            Assert.DoesNotThrow(() => mduFile.Read(testPath, convertedFileObjectsForFMModel), "It Crashed");
        }
        ...
        public void GivenModelForImporting_WhenNcFileIsInSubFolder_ThenNcFileShouldBeAgainInSubFolder(string relativeMduFilePath, string relativeNcFilePath)
        {
            ...
            try
            {
                ...
                var convertedFileObjectsForFMModel = CreateConvertedFileObjectsForFMModel();
                convertedFileObjectsForFMModel.ModelDefinition = originalMD;
                convertedFileObjectsForFMModel.HydroArea = originalArea;
...
*/
        ...
        [Test]
        [Category(TestCategory.DataAccess)]
        public void GivenAnMduToReadWithDryPoints_WhenReadingTheDryPointFile_ThenAreaHasDryPoints()
        {
            mduFilePath = TestHelper.GetTestFilePath(@"ModelWithDrypointData\FlowFM\input_with_drysuffix\FlowFM.mdu");
            mduFilePath = TestHelper.CreateLocalCopy(mduFilePath);
            mduDir = Path.GetDirectoryName(mduFilePath);
            Assert.NotNull(mduDir);
            modelName = Path.GetFileName(mduFilePath);
            ...
            var mduFile = new MduFile();
            var convertedFileObjectsForFMModel = CreateConvertedFileObjectsForFMModel();
            var originalArea = convertedFileObjectsForFMModel.HydroArea;
        ...
        public void Read_WithObsoleteProperty_LogsWarningAndPropertyIsRemovedFromModelDefinition(string property, string iniSection)
        {
            using (var temp = new TemporaryDirectory())
            {
                ...
                // Call
                void Call() => mduFile.Read(mduFilePath, convertedFileObjectsForFMModel);
        ...
        private static ConvertedFileObjectsForFMModel CreateConvertedFileObjectsForFMModel()
        {
