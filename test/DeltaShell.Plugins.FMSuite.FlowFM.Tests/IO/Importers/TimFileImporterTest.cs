...
namespace DeltaShell.Plugins.FMSuite.FlowFM.Tests.IO.Importers
{
    ...
    public class TimFileImporterTest
    {
        ...
        public void GivenTimFileImporterWhenBoundaryConditionHasCorrectDataTypeThenValidate()
        {
            ...
            bool result = new TimFileImporter().CanImportOnBoundaryCondition(flowBoundaryCondition);
        ...
        public void TestImportItem_SourceAndSinks_LogsErrorMessageForNullModel()
        {
            ...
            var importer = new TimFileImporter() {GetModelForSourceAndSink = input => null};

            string expectedLogMessage = string.Format(Resources.Tim_file_import_failed__could_not_retrieve_model_for_SourceAndSink___0_, sourceAndSink.Name);
        ...
        public void TestImportItem_SourceAndSinks_LogsErrorMessageForNullFunction()
        {
            ...
            // do the import & check results
            var importer = new TimFileImporter() {GetModelForSourceAndSink = input => fmModel};

            string expectedLogMessage = string.Format(Resources.Tim_file_import_failed__could_not_retrieve_function_for_SourceAndSink___0_, sourceAndSink.Name);
            TestHelper.AssertAtLeastOneLogMessagesContains(() => importer.ImportItem(string.Empty, sourceAndSink), expectedLogMessage);
        }
        ...
        public void TestImportItem_SourceAndSinks(bool useSalinity, HeatFluxModelType temperature, bool useSedimentMorphology, bool useSecondaryFlow, bool useTracers)
        {
            ...
            // setup
            WaterFlowFMModel fmModel = SetupFMModelWithSourceAndSink(useSalinity, temperature, useSedimentMorphology, useSecondaryFlow, useTracers);
            SourceAndSink sourceAndSink = fmModel.SourcesAndSinks.FirstOrDefault();
        ...
        private WaterFlowFMModel SetupFMModelWithSourceAndSink(bool useSalinity, HeatFluxModelType temperature, bool useSedimentMorphology, bool useSecondaryFlow, bool useTracers)
        {
            ...
            WaterFlowFMProperty salinityProperty = modelDefinition.GetModelProperty(KnownProperties.UseSalinity);
            salinityProperty.Value = useSalinity;

            WaterFlowFMProperty tempertureProperty = modelDefinition.GetModelProperty(KnownProperties.Temperature);
            tempertureProperty.SetValueFromString(((int) temperature).ToString());

            WaterFlowFMProperty sedimentMorphologyProperty = modelDefinition.GetModelProperty(GuiProperties.UseMorSed);
            ...
            WaterFlowFMProperty secondaryFlowProperty = modelDefinition.GetModelProperty(KnownProperties.SecondaryFlow);
        ...
        private void ValidateImportedSourceAndSinkFunction(IFunction function, bool useSalinity, bool useTemperature, bool useSecondaryFlow)
        {
            ...
            if (!useSalinity)
            {
                function.Components.Remove(salinityVariable);
            }

            if (!useTemperature)
            {
                function.Components.Remove(temperatureVariable);
            }

            if (!useSecondaryFlow)
            {
                function.Components.Remove(secondaryFlowVariable);
            }

            // verify values in remaining components (in the test file, the values always increase through the components)
            List<double> allValues = function.Components.SelectMany(c => (MultiDimensionalArray<double>) c.Values).ToList();
            IOrderedEnumerable<double> orderedByAsc = allValues.OrderBy(d => d);
