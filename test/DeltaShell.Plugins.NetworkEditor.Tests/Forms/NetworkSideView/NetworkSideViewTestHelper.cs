...
namespace DeltaShell.Plugins.NetworkEditor.Tests.Forms.NetworkSideView
{
    public static class NetworkSideViewTestHelper
    {
        ...
        public static NetworkSideViewDataController CreateEmptyViewData()
        {
            ...
            return new NetworkSideViewDataController(route, new NetworkSideViewCoverageManager(route, null, new[]
            {
                waterLevel
            }));
        }
        ...
        public static NetworkSideViewDataController CreateDefaultViewDataWithWeir(IHydroNetwork hydroNetwork)
        {
            ...
            INetworkCoverage timeDependendWaterLevelCoverage = CreateTimeDependendWaterLevelCoverage(hydroNetwork, 20);
            IMultiDimensionalArray<DateTime> timeValues = timeDependendWaterLevelCoverage.Time.Values;
            ...
            featureCoverage.Features = new EventedList<IFeature>(new[]
            {
                weir
            });
            featureCoverage.FeatureVariable.SetValues(new[]
            {
                weir
            });
            ...
            var level = 1.0;
            foreach (DateTime timeValue in timeValues)
            {
                featureCoverage.SetValues(new[]
                {
                    level
                }, new VariableValueFilter<DateTime>(timeVariable, timeValue));
                level += delta;
            }
        ...
        public static INetworkCoverage GetTimeDependendNetworkCoverageOnHydroNetwork(int timeSlices, int numberOfBranches, bool storeInNetCdf, string path)
        {
            ...
            IHydroNetwork network = HydroNetworkHelper.GetSnakeHydroNetwork(numberOfBranches, storeInNetCdf);
            ...
            //1..900
            IEnumerable<int> values = Enumerable.Range(1, 9 * numberOfBranches);
        ...
        public static NetworkCoverage GetCoverageBasedOnOtherCoverage(INetworkCoverage sourceCoverage, double initialComponentValue, double componentDelta, Unit unit, string coverageName)
        {
            ...
            foreach (INetworkLocation location in sourceCoverage.Locations.Values)
            {
        ...
        {
            HydroNetwork hydroNetwork = GetDefaultHydroNetwork();
            NetworkSideViewDataController viewData = CreateDefaultViewData(hydroNetwork, null);
