...
{
    [TestFixture]
    public class LateralSourcePropertiesTest
    {
        [Test]
        public void SetName_InvalidName_OriginalNameIsPreserved()
        {
            ...
            var data = new LateralSource { Name = "some_name" };
            data.AttachNameValidator(validator);
            var properties = new LateralSourceProperties { Data = data };
        ...
        {
            WindowsFormsTestHelper.ShowPropertyGridForObject(new LateralSourceProperties { Data = new LateralSource() });
        }
    }
}