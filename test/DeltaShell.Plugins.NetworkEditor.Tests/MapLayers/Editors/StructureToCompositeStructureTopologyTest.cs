...
namespace DeltaShell.Plugins.NetworkEditor.Tests.MapLayers.Editors
{
    ...
    public class StructureToCompositeStructureTopologyTest
    {
        ...
        public void AddPump()
        {
            ...
            IPump pump = network.Pumps.First();
            ICompositeBranchStructure compositeStructure = network.CompositeBranchStructures.First();
        ...
        public void MovePump()
        {
            ...
            hydroNetworkLayer = InitializeNetworkEditor(network);
            ILayer pumpLayer = GetPumpLayer(hydroNetworkLayer);

            IFeature channel = AddGeometry(GetChannelLayer(hydroNetworkLayer), GeometryFromWKT.Parse("LINESTRING (0 0, 100 0)"));
        ...
        public void MovePumpWithToolToOtherBranch()
        {
            var network = new HydroNetwork();
            hydroNetworkLayer = InitializeNetworkEditor(network);
            IFeature channel1 = AddGeometry(GetChannelLayer(hydroNetworkLayer), GeometryFromWKT.Parse("LINESTRING (0 0, 100 0)"));
            IFeature channel2 = AddGeometry(GetChannelLayer(hydroNetworkLayer), GeometryFromWKT.Parse("LINESTRING (100 0, 200 0)"));
            ILayer pumpLayer = GetPumpLayer(hydroNetworkLayer);
        ...
        public void MoveCompositeToOtherBranch()
        {
            ...
            hydroNetwork.Branches.Add(new Channel
            {
            ...
            var channel1 = (IChannel) hydroNetwork.Branches[0];
            var channel2 = (IChannel) hydroNetwork.Branches[1];
            ...
            var compositeStructureEditor = new CompositeStructureInteractor(new VectorLayer {Map = mapControl.Map}, CompositeBranchStructure,
                                                                            new VectorStyle {Symbol = new Bitmap(16, 16)}, hydroNetwork) {Network = hydroNetwork};
