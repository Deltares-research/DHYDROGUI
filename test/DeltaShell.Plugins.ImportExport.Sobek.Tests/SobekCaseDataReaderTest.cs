...
namespace DeltaShell.Plugins.ImportExport.Sobek.Tests
{
    ...
    public class SobekCaseDataReaderTest
    {
        ...
        public void Read_NoRelevantFilePathsSpecified_ReadsCorrectData()
        {
            ...
            // Assert
            Assert.That(caseData.WindDataPath, Is.Null);
            Assert.That(caseData.BuiDataPath, Is.Null);
        }

        [TestCase("FIXED")]
        [TestCase("the\\FIXED")]
        public void Read_ReadsCorrectData(string relFixedDirPath)
        {
            // Setup
            string fileContent =
                @"I \SB216003\FIXED\656NOP.BUI 1331 '1622702956'" + Environment.NewLine +
                @"I \SB216003\FIXED\656NOP.WDC 231 '1402382291'";
            ...
            {
                fixedDirPath = temp.CreateDirectory(relFixedDirPath);
                string rootFilePath = Path.Combine(temp.Path, "the\\root\\file.txt");

                using (var stream = new MemoryStream(Encoding.UTF8.GetBytes(fileContent)))
                {
                    // Call
                    caseData = SobekCaseDataReader.Read(stream, rootFilePath);
                }
            }

            // Assert
            Assert.That(caseData.WindDataPath, Is.EqualTo(Path.Combine(fixedDirPath, "656NOP.WDC")));
            Assert.That(caseData.BuiDataPath, Is.EqualTo(Path.Combine(fixedDirPath, "656NOP.BUI")));
        }
    }
}