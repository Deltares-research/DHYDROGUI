...
namespace DeltaShell.Sobek.Readers.Tests.Readers
{
    ...
    public class SobekCompoundStructureReaderTest
    {
        ...
        {
            var compoundStructureFile = TestHelper.GetTestDataDirectoryPathForAssembly(typeof(SobekWaterFlowModel1DImporterTest).Assembly, @"compound.lit\2\struct.cmp");
        ...
        {
            var compoundStructureFile = TestHelper.GetTestDataDirectoryPathForAssembly(typeof(SobekWaterFlowModel1DImporterTest).Assembly, @"ReModels\RIJN301.SBK\8\defstr.7");
            var compoundStructures = new SobekCompoundStructureReader().Read(compoundStructureFile);
            // the demo network has 1 of each of the 11 main structure types
