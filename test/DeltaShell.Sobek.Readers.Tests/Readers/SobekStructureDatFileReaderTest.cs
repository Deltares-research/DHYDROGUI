...
namespace DeltaShell.Sobek.Readers.Tests.Readers
{
    ...
    public class SobekStructureDatFileReaderTest
    {
        ...
        {
            var pathToSobekNetwork = TestHelper.GetTestDataDirectoryPathForAssembly(typeof(SobekWaterFlowModel1DImporterTest).Assembly, @"ndb_controllertriggerfiles\STRUCT.DAT");
        ...
        {
            var pathToSobekNetwork = TestHelper.GetTestDataDirectoryPathForAssembly(typeof(SobekWaterFlowModel1DImporterTest).Assembly, @"ndb_controllertriggerfiles\DEFSTR.2");
            var structureMappings = new SobekStructureDatFileReader().Read(pathToSobekNetwork).ToArray();
