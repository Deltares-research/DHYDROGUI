...
namespace DeltaShell.Sobek.Readers.Tests.Readers
{
    ...
    public class SobekNetworkStructureReaderTest
    {
        ...
        {
            var structureLocationFile = TestHelper.GetTestDataPath(typeof(SobekWaterFlowModel1DImporterTest).Assembly, @"StrTest.lit\1\network.st");
        ...
        {
            var structureLocationFile = TestHelper.GetTestDataPath(typeof(SobekWaterFlowModel1DImporterTest).Assembly, @"TestXRST.lit\4\network.st");
            var structures = new SobekNetworkStructureReader().Read(structureLocationFile);
        ...
        {
            var structureLocationFile = TestHelper.GetTestDataPath(typeof(SobekWaterFlowModel1DImporterTest).Assembly, @"compound.lit\2\network.st");
            var structures = new SobekNetworkStructureReader().Read(structureLocationFile);
            // the demo network has 1 of each of the 11 main structure types
