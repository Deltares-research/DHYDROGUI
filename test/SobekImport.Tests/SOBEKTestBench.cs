...
using DeltaShell.Plugins.DelftModels.WaterFlowModel.ImportSobek.Tests;
using NUnit.Framework;
using SobekImport.Tests.Helpers;

namespace SobekImport.Tests
{
    [TestFixture]
    [Category(TestCategorySobekImport.WaterFlow1D)]
    public class SOBEKTestBench
    {
        ...
        private static ModelImportAndExportChecker modelImportAndExportChecker;
        ...
        private static void RunTest(int testNumber)
        {
            ...
            if (info != null)
            {
                modelImportAndExportChecker = new ModelImportAndExportChecker(info.CaseDirectory);
                if (modelImportAndExportChecker.ModelToRun != null)
                {
                    var workDirectory = modelImportAndExportChecker.ModelToRun.ExplicitWorkingDirectory;
                    if (modelImportAndExportChecker.ModelToRun.ExplicitWorkingDirectory == null)
                    {
                        var dirPath =
                            Path.GetDirectoryName(modelImportAndExportChecker.ModelToRun.Paths.FirstOrDefault()) ??
                            Environment.CurrentDirectory;
                        workDirectory = Path.Combine(dirPath,
                            modelImportAndExportChecker.ModelToRun.Name.Replace(' ', '_') + "_output");
                    }

                    FileUtils.CreateDirectoryIfNotExists(workDirectory);
                    var dimrFile = Path.Combine(workDirectory, "dimr.xml");

                    var exporter = new DHydroConfigXmlExporter {ExportFilePath = dimrFile};
                    exporter.Export(modelImportAndExportChecker.ModelToRun, dimrFile);
                }
                else
                {
                    Assert.Fail("Not exported");
                    return;

                }
                var exportDir = Path.Combine(TestCasesDirectory, info.TestDirectory);
                var referenceDir = Path.Combine(exportDir, "model_reference");
                exportDir = Path.Combine(exportDir, Environment.Is64BitProcess ? "model_export_x64" : "model_export_x86");
                 
                modelImportAndExportChecker.CopyModelData(exportDir);

                modelImportAndExportChecker.CheckModelExport(exportDir, referenceDir);
        ...
        {
            if (modelImportAndExportChecker != null)
            {
                modelImportAndExportChecker.Dispose();
            }
        }
