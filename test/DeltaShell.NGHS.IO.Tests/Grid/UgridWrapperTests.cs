...
namespace DeltaShell.NGHS.IO.Tests.Grid
{
    ...
    public class UgridWrapperTests
    {
        ...
        private void check2dmesh(int ioncId, int mesh2DId, ref GridWrapper wrapper)
        {
        ...
        private void write1DNetworkAndMesh(int fileId, ref int networkId, ref int mesh1DId, ref GridWrapper wrapper)
        {
            //Mesh variables
            IntPtr c_branchidx = Marshal.AllocCoTaskMem(Marshal.SizeOf(typeof(int)) * nmesh1dPoints);
            IntPtr c_offset = Marshal.AllocCoTaskMem(Marshal.SizeOf(typeof(double)) * nmesh1dPoints);
            ...
            try
            {
                ...
                ierr = wrapper.Write1DNetworkBranches(fileId, networkId, c_sourcenodeid, c_targetnodeid, branchinfo, c_branchlengths, c_nbranchgeometrypoints, nBranches, startIndex);
                Assert.That(ierr, Is.EqualTo(0));
        ...
        public void DeltaShellClones2dMesh()
        {
            ...
            ierr = wrapper.Get2DMeshId(ref sourceFileId, ref sourceNetworkId);
            Assert.That(ierr, Is.EqualTo(0));
            ...
            int targetMesh2D = -1;
            ierr = wrapper.clone_mesh_definition(ref sourceFileId, ref targetFileId, ref sourceMesh2DId, ref targetMesh2D);
            Assert.That(ierr, Is.EqualTo(0));

            //8. Clone the 2d mesh data
            ierr = wrapper.clone_mesh_data(ref sourceFileId, ref targetFileId, ref sourceMesh2DId, ref targetMesh2D);
            Assert.That(ierr, Is.EqualTo(0));
            ...
            ierr = wrapper.Get2DMeshId(ref targetFileId, ref targetMesh2D);
            Assert.That(ierr, Is.EqualTo(0));
