...
namespace DeltaShell.NGHS.IO.Tests.Grid
{
    ...
    public class UGridNetworkApiTests
    {
        ...
        public void CreateNetworkInvalidInitializationTest()
        {
            ...
            uRemoteUGridNetworkApi.Expect(a => a.CreateNetwork(0, 0, 0, out networkId))
            ...
            Assert.AreEqual(GridApiDataSet.GridConstants.GENERAL_FATAL_ERR,
                uGridNetworkApi.CreateNetwork(0, 0, 0, out networkId));

            // uRemoteGridNetworkApi
            Assert.AreEqual(GridApiDataSet.GridConstants.GENERAL_FATAL_ERR,
                uRemoteUGridNetworkApi.CreateNetwork(0, 0, 0, out networkId));
        }
        ...
        {
            // arrange
            string name = GridApiDataSet.DataSetNames.Network;
            int nNodes = 0;
            int nBranches = 0;
            int nGeoPoints = 0;
            ...
            // uRemoteGridNetworkApi
            uRemoteUGridNetworkApi.Expect(a => a.CreateNetwork(nNodes, nBranches, nGeoPoints, out nwid))
                .CallOriginalMethod(OriginalCallOptions.NoExpectation)
                .Repeat.Once();
            ...
            // uGridNetworkApi
            Assert.AreEqual(GridApiDataSet.GridConstants.NOERR,
                uGridNetworkApi.CreateNetwork(nNodes, nBranches, nGeoPoints, out nwid));

            // uRemoteGridNetworkApi
            Assert.AreEqual(GridApiDataSet.GridConstants.NOERR,
                uRemoteUGridNetworkApi.CreateNetwork(nNodes, nBranches, nGeoPoints, out nwid));
        }
        ...
        public void CreateNetworkApiCallFailedTest()
        {
            ...
            // uGridNetworkApi
            Assert.AreEqual(GridApiDataSet.GridConstants.TESTING_ERROR,
                uGridNetworkApi.CreateNetwork(nNodes, nBranches, nGeoPoints, out nwid));
        ...
        public void WriteNetworkNodesInvalidInitializationTest(bool isInitialized, bool isReady)
        {
            ...
            // uRemoteGridNetworkApi
            uRemoteUGridNetworkApi.Expect(
                a => a.WriteNetworkNodes(new double[0], new double[0], new string[0], new string[0]))
                .CallOriginalMethod(OriginalCallOptions.NoExpectation);
            ...
            // uRemoteGridNetworkApi
            var remoteResult = uRemoteUGridNetworkApi.WriteNetworkNodes(new double[0], new double[0], new string[0],
                new string[0]);
        ...
        public void WriteNetworkNodesTest()
        {
            ...
            // uGridNetworkApi
            int nodes;
            uGridNetworkApi.Expect(a => a.GetNumberOfNetworkNodes(1, out nodes))
                .IgnoreArguments()
                .OutRef(nNodes)
                .Return(GridApiDataSet.GridConstants.NOERR)
                .Repeat.Twice();
            ...
            // uRemoteGridNetworkApi
            uRemoteUGridNetworkApi.Expect(a => a.WriteNetworkNodes(nodesX, nodesY, nodesIds, nodesLongnames))
                .CallOriginalMethod(OriginalCallOptions.NoExpectation);
        ...
        [TestCase(0, new[] {0.0}, new[] {0.0}, new[] {"", ""}, new[] {""})]
        [TestCase(0, new[] {0.0}, new[] {0.0}, new[] {""}, new[] {"", ""})]
        public void WriteNetworkNodesInitializedButArrayNotCorrectTest(int numberOfNetworkNodes, double[] nodesX,
            double[] nodesY, string[] nodesIds, string[] nodesLongnames)
        {
            // check with :
            // node Of numbers; 
            // Length xArray; 
            ...
            // uGridNetworkApi
            Assert.AreEqual(GridApiDataSet.GridConstants.GENERAL_ARRAY_LENGTH_FATAL_ERR,
                uGridNetworkApi.WriteNetworkNodes(nodesX, nodesY, nodesIds, nodesLongnames));

            // uRemoteGridNetworkApi
            Assert.AreEqual(GridApiDataSet.GridConstants.GENERAL_ARRAY_LENGTH_FATAL_ERR,
                uRemoteUGridNetworkApi.WriteNetworkNodes(nodesX, nodesY, nodesIds, nodesLongnames));
        }
        ...
        public void WriteNetworkBranchesInvalidInitializationTest(bool isInitialized, bool isReady)
        {
            ...
            // uRemoteGridNetworkApi    
            var remoteResult = uRemoteUGridNetworkApi.WriteNetworkBranches(new int[0], new int[0], new double[0],
                new int[0],
                new string[0], new string[0], new int[0]);
            Assert.AreEqual(GridApiDataSet.GridConstants.GENERAL_FATAL_ERR, remoteResult);
        }
        ...
        public void WriteNetworkBranchesTest()
        {
            ...
            // uGridNetworkApi
            var result = uGridNetworkApi.WriteNetworkBranches(sourceNodeId, targetNodeId, branghLength, nBranchGeoPoints,
                branchId, branchLongnames, branchOrderNumbers);
            Assert.AreEqual(GridApiDataSet.GridConstants.NOERR, result);

            // uRemoteGridNetworkApi
            var remoteResult = uRemoteUGridNetworkApi.WriteNetworkBranches(sourceNodeId, targetNodeId, branghLength,
                nBranchGeoPoints, branchId, branchLongnames, branchOrderNumbers);
            Assert.AreEqual(GridApiDataSet.GridConstants.NOERR, remoteResult);
        }
        ...
        public void WriteNetworkBranchesApiCallFailedTest()
        {
            ...
            uRemoteUGridNetworkApi.Expect(
                a =>
                    a.WriteNetworkBranches(sourceNodeId, targetNodeId, branghLength, nBranchGeoPoints, branchId,
                        branchLongnames, branchOrderNumbers)).CallOriginalMethod(OriginalCallOptions.NoExpectation);
        ...
        public void WriteNetworkBranchesInitializedButArrayNotCorrectTest(int nBranches, int[] sourceNodeId,
            int[] targetNodeId, double[] branchLength, int[] nBranchGeoPoints, string[] branchId,
            string[] branchLongname, int[] branchOrderNumbers)
        {
            // check with :
            // Branch Of numbers; 
            // Length srcIds; 
            ...
            uRemoteUGridNetworkApi.Expect(
                a =>
                    a.WriteNetworkBranches(sourceNodeId, targetNodeId, branchLength, nBranchGeoPoints, branchId,
                        branchLongname, branchOrderNumbers)).CallOriginalMethod(OriginalCallOptions.NoExpectation);

            mocks.ReplayAll();

            // uGridNetworkApi
            Assert.AreEqual(GridApiDataSet.GridConstants.GENERAL_ARRAY_LENGTH_FATAL_ERR,
                uGridNetworkApi.WriteNetworkBranches(sourceNodeId, targetNodeId, branchLength, nBranchGeoPoints,
                    branchId, branchLongname, branchOrderNumbers));

            // uRemoteGridNetworkApi
            Assert.AreEqual(GridApiDataSet.GridConstants.GENERAL_ARRAY_LENGTH_FATAL_ERR,
                uRemoteUGridNetworkApi.WriteNetworkBranches(sourceNodeId, targetNodeId, branchLength, nBranchGeoPoints,
                    branchId, branchLongname, branchOrderNumbers));
        }
        ...
        public void WriteNetworkGeometryTest()
        {
            ...
            // uGridNetworkApi
            int geopoints;
            uGridNetworkApi.Expect(a => a.GetNumberOfNetworkGeometryPoints(1, out geopoints))
                .IgnoreArguments()
                .OutRef(nGeoPoints)
                .Return(GridApiDataSet.GridConstants.NOERR)
                .Repeat.Twice();
            ...
            // uRemoteGridNetworkApi
            uRemoteUGridNetworkApi.Expect(a => a.WriteNetworkGeometry(geopointsX, geopointsY))
                .CallOriginalMethod(OriginalCallOptions.NoExpectation);
        ...
        public void WriteNetworkGeometryInvalidInitializationTest(bool isInitialized, bool isReady)
        {
            ...
            // uRemoteGridNetworkApi
            uRemoteUGridNetworkApi.Expect(a => a.WriteNetworkGeometry(new double[0], new double[0]))
                .CallOriginalMethod(OriginalCallOptions.NoExpectation);
        ...
        public void WriteNetworkNetworkGeometryInitializedButArrayNotCorrectTest(int nGeopoints, double[] geopointsX,
            double[] geopointsY)
        {
            // check with :
            // number of geopoints; 
            ...
            // uGridNetworkApi
            Assert.AreEqual(GridApiDataSet.GridConstants.GENERAL_ARRAY_LENGTH_FATAL_ERR,
                uGridNetworkApi.WriteNetworkGeometry(geopointsX, geopointsY));

            // uRemoteGridNetworkApi
            Assert.AreEqual(GridApiDataSet.GridConstants.GENERAL_ARRAY_LENGTH_FATAL_ERR,
                uRemoteUGridNetworkApi.WriteNetworkGeometry(geopointsX, geopointsY));
        }
        ...
        public void GetNumberOfNetworkNodesTest()
        {
            ...
            // uRemoteGridNetworkApi
            int rNodes;
            uRemoteUGridNetworkApi.Expect(a => a.GetNumberOfNetworkNodes(1, out rNodes))
                .CallOriginalMethod(OriginalCallOptions.NoExpectation);
            ...
            // uRemoteGridNetworkApi
            Assert.AreEqual(GridApiDataSet.GridConstants.NOERR,
                uRemoteUGridNetworkApi.GetNumberOfNetworkNodes(1, out rNodes));
        ...
        {
            // uGridNetworkApi
            uGridNetworkApi.Expect(a => a.GetNumberOfNetworkNodes(Arg<int>.Is.Anything, out Arg<int>.Out(1).Dummy))
                .CallOriginalMethod(OriginalCallOptions.NoExpectation);
            uGridNetworkApi.Expect(a => a.Initialized).Return(false).Repeat.Twice();

            // uRemoteGridNetworkApi
            uRemoteUGridNetworkApi.Expect(
                a => a.GetNumberOfNetworkNodes(Arg<int>.Is.Anything, out Arg<int>.Out(1).Dummy))
                .CallOriginalMethod(OriginalCallOptions.NoExpectation);
            ...
            // uRemoteGridNetworkApi
            var remoteResult = uRemoteUGridNetworkApi.GetNumberOfNetworkNodes(Arg<int>.Is.Anything,
        ...
        public void GetNumberOfNetworkBranchesInvalidInitialization()
        {
            // uGridNetworkApi
            uGridNetworkApi.Expect(a => a.Initialized).Return(false).Repeat.Twice();
            uGridNetworkApi.Expect(a => a.GetNumberOfNetworkBranches(Arg<int>.Is.Anything, out Arg<int>.Out(1).Dummy))
                .CallOriginalMethod(OriginalCallOptions.NoExpectation);

            // uRemoteGridNetworkApi
            uRemoteUGridNetworkApi.Expect(
                a => a.GetNumberOfNetworkBranches(Arg<int>.Is.Anything, out Arg<int>.Out(1).Dummy))
                .CallOriginalMethod(OriginalCallOptions.NoExpectation);
            ...
            // uGridNetworkApi
            Assert.AreEqual(GridApiDataSet.GridConstants.GENERAL_FATAL_ERR,
                uGridNetworkApi.GetNumberOfNetworkBranches(Arg<int>.Is.Anything, out Arg<int>.Out(1).Dummy));

            // uRemoteGridNetworkApi
            Assert.AreEqual(GridApiDataSet.GridConstants.GENERAL_FATAL_ERR,
                uRemoteUGridNetworkApi.GetNumberOfNetworkBranches(Arg<int>.Is.Anything, out Arg<int>.Out(1).Dummy));
        }
        ...
        public void GetNumberOfNetworkGeometryPointsTest()
        {
            ...
            // uRemoteGridNetworkApi
            int rgeopoints;
            uRemoteUGridNetworkApi.Expect(a => a.GetNumberOfNetworkGeometryPoints(1, out rgeopoints))
                .CallOriginalMethod(OriginalCallOptions.NoExpectation);
            ...
            // uGridNetworkApi
            Assert.AreEqual(GridApiDataSet.GridConstants.NOERR,
                uGridNetworkApi.GetNumberOfNetworkGeometryPoints(1, out geopoints));
            Assert.AreEqual(nGeometryPoints, geopoints);

            // uRemoteGridNetworkApi
            Assert.AreEqual(GridApiDataSet.GridConstants.NOERR,
                uRemoteUGridNetworkApi.GetNumberOfNetworkGeometryPoints(1, out rgeopoints));
        ...
        public void GetNumberOfNetworkBranchesTest()
        {
            ...
            uGridNetworkApi.Expect(
                a => a.GetNumberOfNetworkBranches(Arg<int>.Is.Anything, out Arg<int>.Out(nNetworkBranches).Dummy))
                .CallOriginalMethod(OriginalCallOptions.NoExpectation);
            ...
            // uRemoteGridNetworkApi
            uRemoteUGridNetworkApi.Expect(
                a => a.GetNumberOfNetworkBranches(Arg<int>.Is.Anything, out Arg<int>.Out(branches).Dummy))
                .CallOriginalMethod(OriginalCallOptions.NoExpectation);
            ...
            // uGridNetworkApi
            Assert.AreEqual(GridApiDataSet.GridConstants.NOERR,
                uGridNetworkApi.GetNumberOfNetworkBranches(Arg<int>.Is.Anything, out Arg<int>.Out(branches).Dummy));
            Assert.AreEqual(nNetworkBranches, branches);

            // uRemoteGridNetworkApi
            Assert.AreEqual(GridApiDataSet.GridConstants.NOERR,
                uRemoteUGridNetworkApi.GetNumberOfNetworkBranches(Arg<int>.Is.Anything, out Arg<int>.Out(branches).Dummy));
            Assert.AreEqual(nNetworkBranches, branches);
        }
        ...
        public void GetNumberOfNetworkGeometryPointsInvalidInitialization()
        {
            ...
            int geopoints = 0;
            uGridNetworkApi.Expect(
                a => a.GetNumberOfNetworkGeometryPoints(Arg<int>.Is.Anything, out Arg<int>.Out(geopoints).Dummy))
                .CallOriginalMethod(OriginalCallOptions.NoExpectation);
            uGridNetworkApi.Expect(a => a.Initialized).Return(false).Repeat.Twice();
            ...
            // uGridNetworkApi
            Assert.AreEqual(GridApiDataSet.GridConstants.GENERAL_FATAL_ERR,
                uGridNetworkApi.GetNumberOfNetworkGeometryPoints(Arg<int>.Is.Anything, out Arg<int>.Out(geopoints).Dummy));
        ...
        public void GetNumberOfNetworkGeometryPointsApiCallFailed()
        {
            ...
            // uRemoteGridNetworkApi
            Assert.AreEqual(GridApiDataSet.GridConstants.TESTING_ERROR,
                uGridNetworkApi.GetNumberOfNetworkGeometryPoints(1, out rgeopoints));
        }
        ...
        public void ReadNetworkNodesInvalidInitializationTest()
        {
            ...
            // uRemoteGridNetworkApi
            uRemoteUGridNetworkApi.Expect(
                a => a.ReadNetworkNodes(1, out nodesX, out nodesY, out nodesIds, out nodesLongnames))
                .CallOriginalMethod(OriginalCallOptions.NoExpectation);
            ...
            // uGridNetworkApi
            Assert.AreEqual(GridApiDataSet.GridConstants.GENERAL_FATAL_ERR,
                uGridNetworkApi.ReadNetworkNodes(1, out nodesX, out nodesY, out nodesIds, out nodesLongnames));

            // uRemoteGridNetworkApi
            Assert.AreEqual(GridApiDataSet.GridConstants.GENERAL_FATAL_ERR,
                uRemoteUGridNetworkApi.ReadNetworkNodes(1, out nodesX, out nodesY, out nodesIds, out nodesLongnames));
        }
        ...
        public void ReadNetworkNodesTest()
        {
            ...
            // uRemoteGridNetworkApi
            var remoteResult = uRemoteUGridNetworkApi.ReadNetworkNodes(1, out nodesX, out nodesY, out nodesIds,
                out nodesLongnames);
            Assert.AreEqual(GridApiDataSet.GridConstants.NOERR, remoteResult);
        }
        ...
        public void ReadNetworkBranchesInvalidInitializationTest()
        {
            ...
            uRemoteUGridNetworkApi.Expect(
                a =>
                    a.ReadNetworkBranches(1, out sourceNodes, out targetNodes, out branchLengths, out branchGeoPoints,
                        out branchIds, out branchLongnames, out branchOrderNumbers))
                .CallOriginalMethod(OriginalCallOptions.NoExpectation);

            mocks.ReplayAll();

            // uGridNetworkApi
            var result = uGridNetworkApi.ReadNetworkBranches(1, out sourceNodes, out targetNodes, out branchLengths,
                out branchGeoPoints, out branchIds, out branchLongnames, out branchOrderNumbers);
            Assert.AreEqual(GridApiDataSet.GridConstants.GENERAL_FATAL_ERR, result);

            // uRemoteGridNetworkApi
            var remoteResult = uRemoteUGridNetworkApi.ReadNetworkBranches(1, out sourceNodes, out targetNodes,
        ...
        public void ReadNetworkGeometryInvalidInitializationTest()
        {
            ...
            // uRemoteGridNetworkApi
            uRemoteUGridNetworkApi.Expect(a => a.ReadNetworkGeometry(1, out geopointsX, out geopointsY))
                .CallOriginalMethod(OriginalCallOptions.NoExpectation);
            ...
            // uGridNetworkApi
            Assert.AreEqual(GridApiDataSet.GridConstants.GENERAL_FATAL_ERR,
                uGridNetworkApi.ReadNetworkGeometry(1, out geopointsX, out geopointsY));
            Assert.AreEqual(0, geopointsX.Length);
            Assert.AreEqual(0, geopointsY.Length);

            // uRemoteNetworkApi
            Assert.AreEqual(GridApiDataSet.GridConstants.GENERAL_FATAL_ERR,
                uRemoteUGridNetworkApi.ReadNetworkGeometry(1, out geopointsX, out geopointsY));
            Assert.AreEqual(0, geopointsX.Length);
            Assert.AreEqual(0, geopointsY.Length);
        }
        ...
        public void ReadNetworkGeometryTest()
        {
            ...
            uGridNetworkApi.Expect(
                a => a.GetNumberOfNetworkGeometryPoints(Arg<int>.Is.Anything, out Arg<int>.Out(nGeoPoints).Dummy))
                .Return(GridApiDataSet.GridConstants.NOERR)
                .Repeat.Twice();
        ...
        public void NetworkReadyTrueTest()
        {
            ...
            // uRemoteNetworkApi
            uRemoteUGridNetworkApi.Expect(a => a.NetworkReadyForWriting)
                .CallOriginalMethod(OriginalCallOptions.NoExpectation);
        ...
        {
            //uRemoteGridNetworkApi
            uRemoteUGridNetworkDiscretisationApi.Expect(a => a.CreateNetworkDiscretisation(0,0))
                .CallOriginalMethod(OriginalCallOptions.NoExpectation);
            ...
            // uGridNetworkApi
            Assert.AreEqual(GridApiDataSet.GridConstants.GENERAL_FATAL_ERR,
                uGridNetworkDiscretisationApi.CreateNetworkDiscretisation(0,0));

            // uRemoteGridNetworkApi
            Assert.AreEqual(GridApiDataSet.GridConstants.GENERAL_FATAL_ERR,
                uRemoteUGridNetworkDiscretisationApi.CreateNetworkDiscretisation(0, 0));
        }
        ...
        public void CreateNetworkDiscretisationTest()
        {
            ...
            wrapper.Expect(w => w.Create1DMesh(id, GridApiDataSet.DataSetNames.Network, ref meshId, GridApiDataSet.DataSetNames.Mesh1D, nmeshpts, nmeshedges))
                .OutRef(id, meshId, nmeshpts, nmeshedges).IgnoreArguments()
                .Return(GridApiDataSet.GridConstants.NOERR).Repeat.Twice();

            // uRemoteGridNetworkApi
            uRemoteUGridNetworkDiscretisationApi.Expect(
                a =>
                    a.CreateNetworkDiscretisation(Arg<int>.Is.Anything, Arg<int>.Is.Anything)).CallOriginalMethod(OriginalCallOptions.NoExpectation);

            mocks.ReplayAll();

            // uGridNetworkApi
            var result = uGridNetworkDiscretisationApi.CreateNetworkDiscretisation(Arg<int>.Is.Anything, Arg<int>.Is.Anything);
            Assert.AreEqual(GridApiDataSet.GridConstants.NOERR, result);

            // uRemoteGridNetworkApi
            var remoteResult = uRemoteUGridNetworkDiscretisationApi.CreateNetworkDiscretisation(
                Arg<int>.Is.Anything, Arg<int>.Is.Anything);
            Assert.AreEqual(GridApiDataSet.GridConstants.NOERR, remoteResult);
        }
        ...
        public void CreateNetworkDiscretisationApiCallFailedTest()
        {
            ...
            // uRemoteGridNetworkApi
            var remoteResult = uRemoteUGridNetworkDiscretisationApi.CreateNetworkDiscretisation(Arg<int>.Is.Anything, Arg<int>.Is.Anything);
            Assert.AreEqual(GridApiDataSet.GridConstants.TESTING_ERROR, remoteResult);
        }
        ...
        public void CreateNetworkDiscretisationExceptionTest()
        {
            ...
            int id = 1;
            int meshId = 0;
            string meshName = GridApiDataSet.DataSetNames.Mesh1D;
            ...
            // uRemoteGridNetworkApi
            uRemoteUGridNetworkDiscretisationApi.Expect(
                a => a.CreateNetworkDiscretisation(nmeshpts, Arg<int>.Is.Anything))
                .CallOriginalMethod(OriginalCallOptions.NoExpectation);
            ...
            // uRemoteGridNetworkApi
            var remoteResult = uRemoteUGridNetworkDiscretisationApi.CreateNetworkDiscretisation(nmeshpts,nmeshedges);
            Assert.AreEqual(GridApiDataSet.GridConstants.GENERAL_FATAL_ERR, remoteResult);
        }
        ...
        public void WriteNetworkDiscretisationPointsInvalidInitializationTest(bool isInitialized, bool isReady)
        {
            ...
            int[] edgeIdx = new int[1];
            double[] edgeOffset = new double[1];
            double[] edgeX = new double[1];
            double[] edgeY = new double[1];
            int[] edgeNodes = new int[1];
            ...
            uRemoteUGridNetworkDiscretisationApi.Expect(
                a => a.WriteNetworkDiscretisationPoints(branchIdx, offset, discretisationPointsX, discretisationPointsY,
                    edgeIdx,edgeOffset,edgeX,edgeY, edgeNodes, 
                    ids, names))
                .CallOriginalMethod(OriginalCallOptions.NoExpectation);
            ...
            var result = uGridNetworkDiscretisationApi.WriteNetworkDiscretisationPoints(branchIdx, offset,
                discretisationPointsX, discretisationPointsY,
                edgeIdx, edgeOffset, edgeX, edgeY, edgeNodes,
                ids, names);
            Assert.AreEqual(GridApiDataSet.GridConstants.GENERAL_FATAL_ERR, result);
        ...
        public void WriteNetworkDiscretisationInitializedButArrayNotCorrectTest(int[] branchIdx, double[] offset)
        {
            ...
            // uRemoteGridNetworkApi
            uRemoteUGridNetworkDiscretisationApi.Expect(
                a => a.WriteNetworkDiscretisationPoints(branchIdx, offset, discretisationPointsX, discretisationPointsY, edgeIdx, edgeOffset, edgeX, edgeY, edgeNodes,
                    ids, names))
                .CallOriginalMethod(OriginalCallOptions.NoExpectation);
            ...
            // uGridNetworkApi
            var result = uGridNetworkDiscretisationApi.WriteNetworkDiscretisationPoints(branchIdx, offset, discretisationPointsX, discretisationPointsY, edgeIdx, edgeOffset, edgeX, edgeY, edgeNodes, ids, names);
            Assert.AreEqual(GridApiDataSet.GridConstants.GENERAL_ARRAY_LENGTH_FATAL_ERR, result);

            // uRemoteGridNetworkApi
            var remoteResult = uRemoteUGridNetworkDiscretisationApi.WriteNetworkDiscretisationPoints(branchIdx, offset, discretisationPointsX, discretisationPointsY, edgeIdx, edgeOffset, edgeX, edgeY, edgeNodes,
                ids, names);
        ...
        public void WriteNetworkDiscretisationTest()
        {
            ...
            // uRemoteGridNetworkApi
            uRemoteUGridNetworkDiscretisationApi.Expect(
                a => a.WriteNetworkDiscretisationPoints(branchIdx, offset, discretisationPointsX, discretisationPointsY, edgeIdx, edgeOffset, edgeX, edgeY, edgeNodes, ids, names))
                .CallOriginalMethod(OriginalCallOptions.NoExpectation);
        ...
        #region Write 1D2DLinks

        public void Write1D2DLinksTest()
        {
            int[] branchIdx = new int[] {1, 1,};
            double[] offset = new double[] {0.5, 1.2,};
            double[] discretisationPointsX = new double[] { 0, 10.0 };
