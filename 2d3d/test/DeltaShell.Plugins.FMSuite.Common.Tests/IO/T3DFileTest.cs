...
namespace DeltaShell.Plugins.FMSuite.Common.Tests.IO
{
    ...
    public class T3DFileTest
    {
        ...
        {
            string path = TestHelper.GetTestFilePath(@"timFiles\testFile.t3D");
            Assert.IsTrue(File.Exists(path));

            var reader = new T3DFile();
            VerticalProfileDefinition verticalProfileDefinition;
            TimeSeries data = reader.Read(path, out verticalProfileDefinition);
            ...
            Assert.AreEqual(new[]
            {
                ...
                50.0,
                100.0
            }, verticalProfileDefinition.SortedPointDepths);
            ...
            Assert.AreEqual(
                ...
                {
                    refDate,
                    refDate + new TimeSpan(0, 0, 0, 9999999)
                },
            ...
            Assert.AreEqual(new[]
            {
                30.0,
                30.0
            }, data.Components[2].Values);
        }

        [Test]
        public void ReadT3DFileSalinity()
        {
            string path = TestHelper.GetTestFilePath(@"timFiles\salinity.t3D");
            Assert.IsTrue(File.Exists(path));
        ...
        public void WriteReadSigmaLayeredProfileData()
        {
            ...
            outputData.Arguments[0].Values.AddRange(new[]
            {
                refDate,
                refDate.AddHours(1),
                refDate.AddHours(1).AddMinutes(5)
            });
            outputData.Components[0].SetValues(new[]
            {
                10.0,
                12.1,
                14.2
            });
            outputData.Components[1].SetValues(new[]
            {
                11.3,
                14.2,
                13.4
            });
            outputData.Components[2].SetValues(new[]
            {
                12.6,
                16.3,
                12.6
            });
            outputData.Components[3].SetValues(new[]
            {
                13.9,
                18.4,
                11.8
            });
            outputData.Components[4].SetValues(new[]
            {
                14.2,
                10.5,
                10.0
            ...
            TimeSeries inputData = writer.Read("test3DFile", out inputProfile);
