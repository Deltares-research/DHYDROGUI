...
{
    [TestFixture]
    public class DirectionSpaceTypeToBooleanConverterTest
    {
        private DirectionSpaceTypeToBooleanConverter converter;

        [SetUp]
        public void Setup()
        {
            converter = new DirectionSpaceTypeToBooleanConverter();
        }
        ...
        {
            yield return new TestCaseData(DirectionalSpaceType.Circle, typeof(bool), false);
            yield return new TestCaseData(DirectionalSpaceType.Sector, typeof(bool), true);

            yield return new TestCaseData(new object(), typeof(bool), DependencyProperty.UnsetValue);

            yield return new TestCaseData(DirectionalSpaceType.Circle, typeof(object), DependencyProperty.UnsetValue);
            yield return new TestCaseData(DirectionalSpaceType.Sector, typeof(object), DependencyProperty.UnsetValue);
        }
    }
}