...
{
    public class SobekNetworkNodeFileReader : SobekReader<SobekNode>
    {
        ...
        private static SobekNode GetNode(string line)
        {
            ...
            if (matches.Count == 1)
            {
                sobekNode.Name = matches[0].Groups[label].Value;
            }
            ...
            if (matches.Count == 1)
            {
                sobekNode.X = Convert.ToDouble(matches[0].Groups[label].Value, CultureInfo.InvariantCulture);
            }
            ...
            if (matches.Count == 1)
            {
                sobekNode.Y = Convert.ToDouble(matches[0].Groups[label].Value, CultureInfo.InvariantCulture);
            }
