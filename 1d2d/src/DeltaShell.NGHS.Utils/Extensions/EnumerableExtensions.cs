...
namespace DeltaShell.NGHS.Utils.Extensions
{
    ...
    public static class EnumerableExtensions
    {
        ...
        public static void ForEach<T1, T2>(this (IEnumerable<T1>, IEnumerable<T2>) sources, Action<T1, T2> action)
        {
            ...
            Ensure.NotNull<IEnumerable<T1>>(sources.Item1, $"{nameof(sources)}.{nameof(sources.Item1)}");
            Ensure.NotNull<IEnumerable<T2>>(sources.Item2, $"{nameof(sources)}.{nameof(sources.Item2)}");
