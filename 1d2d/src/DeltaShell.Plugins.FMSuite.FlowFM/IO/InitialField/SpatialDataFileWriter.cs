...
namespace DeltaShell.Plugins.FMSuite.FlowFM.IO.InitialField
{
    ...
    public sealed class SpatialDataFileWriter : ISpatialDataFileWriter
    {
        ...
        public void Write(string directory, bool switchTo, InitialFieldFileData data, WaterFlowFMModelDefinition definition)
        {
            ...
            IEnumerable<InitialFieldData> spatialDataFields = data.AllFields.Where(x => x.DataFileType != InitialFieldDataFileType.OneDField);
            
            foreach (InitialFieldData initialField in spatialDataFields)
            {
