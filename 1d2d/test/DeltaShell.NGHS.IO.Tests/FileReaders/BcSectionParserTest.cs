...
{
    [TestFixture]
    public class BcSectionParserTest
    {
        ...
        public void SetUp()
        {
            ...
            parser = new BcSectionParser(logHandler);
        }
        ...
        {
            // Call
            void Call() => new BcSectionParser(null);
        ...
        public void CreateConstant_TableHasNoValues_LogsMessageAndReturnsZero(IList<IBcQuantityData> table)
        {
            // Call
        ...
        {
            IList<IBcQuantityData> table = CreateValidTableSubstitute("randomValue");
        ...
        {
            // Arrange
            IList<IBcQuantityData> table = CreateValidTableSubstitute(insertedString);

            // Act & Assert
            Assert.That(parser.CreateConstant(table, randomLineNumber), Is.EqualTo(expectedDouble));
        }

        private static IList<IBcQuantityData> CreateValidTableSubstitute(string value)
        {
            var table = Substitute.For<IList<IBcQuantityData>>();
            
            var dataEnumerator = Substitute.For<IEnumerator<IBcQuantityData>>();
            ...
            var data = Substitute.For<IBcQuantityData>();
        ...
        public void WhenParsingDoubles_WhenInvalidData_ThenReturnExpectedDouble()
        {
            ...
            // Assert
            logHandler.Received(1).ReportError($"Cannot parse 'InvalidData' to a double, see section on line {randomLineNumber}.");
        }
        ...
        {
            yield return new TestCaseData(timeToAdd.ToString(), "centuries since 2021-01-01 00:00:00", $"Cannot interpret 'centuries since 2021-01-01 00:00:00', see section on line {randomLineNumber}.");
            yield return new TestCaseData(timeToAdd.ToString(), "minutes since yesterday", $"Cannot parse 'yesterday' to a date time, see section on line {randomLineNumber}.");
        }
