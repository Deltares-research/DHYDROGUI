...
namespace DeltaShell.Plugins.FMSuite.Common.Tests.IO
{
    ...
    public class WaterFlowFMParserTest
    {
        ...
        {
            var captionField = "";
            Assert.AreEqual(typeof(int), DataTypeValueParser.GetClrType(null, "Integer", ref captionField, null, 0));
            ...
            Assert.AreEqual(typeof(bool), DataTypeValueParser.GetClrType(null, "0|1", ref captionField, null, 0));
            Assert.AreEqual(typeof(bool), DataTypeValueParser.GetClrType(null, "1|0", ref captionField, null, 0));
            Assert.AreEqual(typeof(Steerable), DataTypeValueParser.GetClrType(null, "Steerable", ref captionField, null, 0));
            captionField = "Test:1|2|3|4";
            Assert.IsNotNull(DataTypeValueParser.GetClrType("TestVar", "T|e|s|t", ref captionField, null, 0));
            captionField = "Test:a|b|c|d";
            Assert.IsNotNull(DataTypeValueParser.GetClrType("TestVar", "T|e|s|t", ref captionField, null, 0));

            captionField = "It's a syntax error if no colon is specified when using '|' characters";
            Assert.Throws<FormatException>(() => DataTypeValueParser.GetClrType(null, "T|e|s|t", ref captionField, null, 0), "");
            captionField = "Syntax error: Number of '|' characters should be the same as in typefield";
            Assert.Throws<FormatException>(() => DataTypeValueParser.GetClrType(null, "T|e|s|t", ref captionField, null, 0), "");
            Assert.Throws<ArgumentException>(() => DataTypeValueParser.GetClrType(null, "I am not defined", ref captionField, null, 0), "");
        }

        [Test]
        public void GetStringArrayTest()
        {
            var list = DataTypeValueParser.FromString<IList<string>>("");
            Assert.AreEqual(0, list.Count);

            #region FromString
            // Can deal with space separated:
            list = DataTypeValueParser.FromString<IList<string>>("alpha beta gamma delta epsilon");
            ...
            var newList = DataTypeValueParser.ToString(stringList, typeof(IList<string>));
        ...
        {
            var list = DataTypeValueParser.FromString<IList<double>>("");
            Assert.AreEqual(0, list.Count);

            // Can deal with ints:
            list = DataTypeValueParser.FromString<IList<double>>("1");
            Assert.AreEqual(1, list.Count);
            Assert.AreEqual(new[]{1.0}, list);

            // Always read as Culture Invariant:
            list = DataTypeValueParser.FromString<IList<double>>("1.000");
            Assert.AreEqual(1, list.Count);
            Assert.AreEqual(new[] { 1.0 }, list);
            list = DataTypeValueParser.FromString<IList<double>>("1,000");
            Assert.AreEqual(1, list.Count);
            ...
            // Can deal with space separated:
            list = DataTypeValueParser.FromString<IList<double>>("1.0 2 3.2 4 5.4");
            Assert.AreEqual(5, list.Count);
            ...
            Assert.Throws<ArgumentNullException>(() => DataTypeValueParser.FromString<IList<double>>(null));
        }
        ...
        {
            Assert.AreEqual("1.2", DataTypeValueParser.ToString(1.2, typeof(double)));
            Assert.AreEqual("1.2 3.4", DataTypeValueParser.ToString(new List<double>(new[]{1.2, 3.4}), typeof(IList<double>)));
        }
        ...
        public void ToStringForSteerable()
        {
            ...
            Assert.AreEqual("1.2", DataTypeValueParser.ToString(steerable, typeof(Steerable)));

            steerable.Mode = SteerableMode.TimeSeries;

            Assert.AreEqual("weir01_crest_level.tim", DataTypeValueParser.ToString(steerable, typeof(Steerable)));

            steerable.Mode = SteerableMode.External;

            Assert.AreEqual("REALTIME", DataTypeValueParser.ToString(steerable, typeof(Steerable)));
        }
        ...
        public void FromStringTest()
        {
            ...
            var dataType = DataTypeValueParser.GetClrType("TestVar", "T|e|s|t", ref captionField, null, 0);
        ...
        public static void DateTimeFromString_WhenInputIsNullOrEmpty_ReturnsNow()
        {
            ...
            DateTime result = (DateTime)DataTypeValueParser.FromString("", typeof(DateTime));
        ...
        {
            Assert.That(DataTypeValueParser.ToString(givenDateTime, typeof(DateTime)), Is.EqualTo(expectedDateTime));
        }
        ...
        public static void DateOnlyFromString_WhenInputIsNullOrEmpty_ReturnsNow()
        {
            ...
            var result = (DateOnly)DataTypeValueParser.FromString("", typeof(DateOnly));
        ...
        {
            Assert.That(DataTypeValueParser.ToString(givenDateTime, typeof(DateOnly)), Is.EqualTo(expectedDateTime));
        }
