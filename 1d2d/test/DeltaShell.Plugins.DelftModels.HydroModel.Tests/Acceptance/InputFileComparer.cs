...
namespace DeltaShell.Plugins.DelftModels.HydroModel.Tests.Acceptance
{
    public static class InputFileComparer
    {
        ...
        /// <param name="hasRrData">Whether Rainfall Runoff data should be compared.</param>
                                                   ...
                                                   IReadOnlyDictionary<string, IEnumerable<string>> rainfallRunoffLinesToIgnorePerFile)
        {
            ...
            string flowFmInitialSavePath = Directory.GetFiles(expectedSaveProjectDirectory, mduFileName + ".mdu", SearchOption.AllDirectories).FirstOrDefault();
            string flowFmSecondSavePath = Directory.GetFiles(actualSaveProjectDirectory, mduFileName + ".mdu", SearchOption.AllDirectories).FirstOrDefault();
            
            if (string.IsNullOrEmpty(flowFmInitialSavePath))
            {
                Assert.Fail($"No saved MDU file (first save) could be found at {expectedSaveProjectDirectory}.");
            }
            if (string.IsNullOrEmpty(flowFmSecondSavePath))
            {
                Assert.Fail($"No saved MDU file (seond save) could be found at {actualSaveProjectDirectory}.");
            }
            
            CompareFlowFMInputFiles(Path.GetDirectoryName(flowFmInitialSavePath), Path.GetDirectoryName(flowFmSecondSavePath), tempDirectory, flowFmLinesToIgnorePerFile);
                                                    ...
                                                    IReadOnlyDictionary<string, IEnumerable<string>> flowFmLinesToIgnorePerFile)
        {
            ...
            Assert.Multiple(() => FlowFmFileComparer.Compare(flowFmInitialSaveFiles, flowFmSecondSaveFiles, tempDirectory, flowFmLinesToIgnorePerFile));
        }
