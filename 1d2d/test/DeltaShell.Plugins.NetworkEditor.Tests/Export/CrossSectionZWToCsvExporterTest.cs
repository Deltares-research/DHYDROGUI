...
{
    [TestFixture]
    public class CrossSectionZWToCsvExporterTest
    {
        [Test]
        [Category(TestCategory.DataAccess)]
        public void ExportCrossSectionZWToCsv()
        {
            ...
            Assert.IsTrue(file[0].StartsWith("id,Name,Data_type"));
            Assert.IsTrue(file[1].StartsWith("cs2,,meta"));
            Assert.IsTrue(file[2].StartsWith("cs2,,geom"));
        }

        [Test]
        [Category(TestCategory.DataAccess)]
        public void ExportCrossSectionZWWithProxyDefinitionToCsv()
        {
            var network = HydroNetworkHelper.GetSnakeHydroNetwork(4);
            var branch = network.Branches[1];
