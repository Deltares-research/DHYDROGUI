...
{
    [TestFixture]
    public class WeirPropertyInfoCreationContextTest
    {
        [Test]
        [TestCaseSource(nameof(CustomizeProperties_AnyArgNullCases))]
        public void CustomizeProperties_AnyArgNull_ThrowsArgumentNullException(WeirProperties properties, GuiContainer guiContainer)
        {
            // Setup
            var creationContext = new WeirPropertyInfoCreationContext();
        ...
        {
            var properties = new WeirProperties();
            var guiContainer = new GuiContainer();
            var creationContext = new WeirPropertyInfoCreationContext();

            var hydroNetwork = Substitute.For<IHydroNetwork>();
            IWeir[] features = { GetFeature(hydroNetwork, "feat1"), GetFeature(hydroNetwork, "feat2"), GetFeature(hydroNetwork, "feat3") };
            hydroNetwork.Weirs.Returns(features);

            IWeir propertyData = features[0];
        ...
        private static IWeir GetFeature(IHydroNetwork hydroNetwork, string name)
        {
            var feature = Substitute.For<IWeir>();
            feature.Name = name;
        ...
        public static IEnumerable<TestCaseData> CustomizeProperties_AnyArgNullCases()
        {
            ...
            yield return new TestCaseData(new WeirProperties(), null);
        }
    }
}