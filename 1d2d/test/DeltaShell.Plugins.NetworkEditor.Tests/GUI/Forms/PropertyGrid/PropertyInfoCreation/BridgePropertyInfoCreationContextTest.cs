...
{
    [TestFixture]
    public class BridgePropertyInfoCreationContextTest
    {
        [Test]
        [TestCaseSource(nameof(CustomizeProperties_AnyArgNullCases))]
        public void CustomizeProperties_AnyArgNull_ThrowsArgumentNullException(BridgeProperties properties, GuiContainer guiContainer)
        {
            // Setup
            var creationContext = new BridgePropertyInfoCreationContext();
        ...
        public void SetName_AfterCustomizeProperties_ValidatesNameForUniqueness()
        {
            ...
            IBridge[] features = { GetFeature(hydroNetwork, "feat1"), GetFeature(hydroNetwork, "feat2"), GetFeature(hydroNetwork, "feat3") };
            hydroNetwork.Bridges.Returns(features);

            IBridge propertyData = features[0];
            var properties = new BridgeProperties { Data = propertyData };
        ...
        private static IBridge GetFeature(IHydroNetwork hydroNetwork, string name)
        {
            var feature = Substitute.For<IBridge>();
            feature.Name = name;
        ...
        public static IEnumerable<TestCaseData> CustomizeProperties_AnyArgNullCases()
        {
            ...
            yield return new TestCaseData(new BridgeProperties(), null);
        }
    }
}