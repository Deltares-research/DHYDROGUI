...
{
    [TestFixture]
    public class CrossSectionPropertyInfoCreationContextTest
    {
        [Test]
        [TestCaseSource(nameof(CustomizeProperties_AnyArgNullCases))]
        public void CustomizeProperties_AnyArgNull_ThrowsArgumentNullException(CrossSectionProperties properties, GuiContainer guiContainer)
        {
            // Setup
            var creationContext = new CrossSectionPropertyInfoCreationContext();
        ...
        public void SetName_AfterCustomizeProperties_ValidatesNameForUniqueness()
        {
            ...
            ICrossSection[] features = { GetFeature(hydroNetwork, "feat1"), GetFeature(hydroNetwork, "feat2"), GetFeature(hydroNetwork, "feat3") };
            hydroNetwork.CrossSections.Returns(features);

            ICrossSection propertyData = features[0];
            var properties = new CrossSectionProperties { Data = propertyData };
        ...
        private static ICrossSection GetFeature(IHydroNetwork hydroNetwork, string name)
        {
        ...
        public static IEnumerable<TestCaseData> CustomizeProperties_AnyArgNullCases()
        {
            ...
            yield return new TestCaseData(new CrossSectionProperties(), null);
        }
    }
}