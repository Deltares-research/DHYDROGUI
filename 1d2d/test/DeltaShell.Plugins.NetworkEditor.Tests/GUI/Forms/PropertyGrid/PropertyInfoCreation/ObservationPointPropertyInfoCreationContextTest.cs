...
{
    [TestFixture]
    public class ObservationPointPropertyInfoCreationContextTest
    {
        [Test]
        [TestCaseSource(nameof(CustomizeProperties_AnyArgNullCases))]
        public void CustomizeProperties_AnyArgNull_ThrowsArgumentNullException(ObservationPointProperties properties, GuiContainer guiContainer)
        {
            // Setup
            var creationContext = new ObservationPointPropertyInfoCreationContext();
        ...
        {
            var properties = new ObservationPointProperties();
            var guiContainer = new GuiContainer();
            var creationContext = new ObservationPointPropertyInfoCreationContext();

            var hydroNetwork = Substitute.For<IHydroNetwork>();
            ObservationPoint[] features = { GetFeature(hydroNetwork, "feat1"), GetFeature(hydroNetwork, "feat2"), GetFeature(hydroNetwork, "feat3") };
            hydroNetwork.ObservationPoints.Returns(features);

            ObservationPoint propertyData = features[0];
        ...
        private static ObservationPoint GetFeature(IHydroNetwork hydroNetwork, string name)
        {
            var feature = Substitute.For<ObservationPoint>();
            feature.Name = name;
        ...
        public static IEnumerable<TestCaseData> CustomizeProperties_AnyArgNullCases()
        {
            ...
            yield return new TestCaseData(new ObservationPointProperties(), null);
        }
    }
}