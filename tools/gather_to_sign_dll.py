...
def determine_to_sign_dll_names(svn_root: Path) -> Generator[str, None, None]:
    """
...
def get_corresponding_path(svn_root: Path, dll_name: str) -> Path:
    """
    Get the dll path corresponding with the dll_name from the provided svn_root.
...
def find_all_dll_paths(svn_root: Path, dll_names: Generator[str, None, None]) -> Generator[Path, None, None]:
    """
    ...
    """
    return (get_corresponding_path(svn_root, x) for x in dll_names)


def generate_json_mapping_content(svn_root: Path, dll_paths: Generator[Path, None, None]) -> dict:
    """
    ...
    return { "mapping": list(( { "dll": p.name
                               , "path": str(p.relative_to(svn_root)) 
                               } for p in dll_paths )) 
           }
...
def restore_signed_dlls(svn_root: Path, dll_mapping: dict, sign_dir) -> None:
    """
    ...
    for dll_info in dll_mapping["mapping"]:
        shutil.move(str(sign_dir / dll_info["dll"]), 
                    str(svn_root / Path(dll_info["path"])))


TO_SIGN_DIR_NAME = "to_sign"

def harvest(svn_root: Path) -> None:
    """
    Create a to_sign folder containing all the dlls to sign, and their corresponding mapping

    Args:
        svn_root: Path to the svn root.
    """
    dll_names = determine_to_sign_dll_names(svn_root)
    dll_paths = list(find_all_dll_paths(svn_root, dll_names))

    sign_dir = svn_root / Path(TO_SIGN_DIR_NAME)
    if (not (sign_dir.exists() and sign_dir.is_dir())):
        sign_dir.mkdir(parents=True)

    dll_mapping = generate_json_mapping_content(svn_root, dll_paths)
    write_json_mapping_file(sign_dir, dll_mapping)

    harvest_to_sign_dlls(dll_paths, sign_dir)


def restore(svn_root: Path) -> None:
    """
    Restore the signed dlls to their original locations.
    ...
    restore_signed_dlls(svn_root, dll_mapping, sign_dir)
...
if __name__ == "__main__":
    ...
    svn_root = Path(args.svn_root_path)
